    ...
    <div class="space-y-6 w-full">
      ...
      <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 pb-4 border-b border-border">
        <div class="flex items-center gap-3">
          <div class="size-12 rounded-2xl bg-primary/10 text-primary flex items-center justify-center shrink-0 border border-primary/20 shadow-xs">
            <ng-icon name="lucideUserRound" size="24" />
          </div>
          <div>
            <div class="flex items-center gap-2 flex-wrap">
              <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-foreground">My Health Profile & Clinical Onboarding</h1>
              <span hlmBadge variant="outline" class="font-mono text-[10px]">MRN: {{ patient()?.patientCode || 'PAT-1001' }}</span>
            </div>
            <p class="text-xs text-muted-foreground mt-0.5">
              Unified personal health record (PHR) chart setup, emergency contacts, insurance verification, and medical safety profile.
            </p>
          </div>
        </div>
      ...
      <div class="p-5 rounded-2xl border border-primary/20 bg-primary/5 space-y-3 shadow-xs">
        ...
        <div class="flex items-center flex-wrap gap-2 pt-2 border-t border-primary/10 text-[11px]">
          ...
          <button (click)="activeTab.set('demographics')" [class.bg-emerald-500\/10]="hasDemographics()" class="px-2.5 py-1 rounded-lg border border-border flex items-center gap-1.5 hover:bg-accent transition-colors">
            <ng-icon [name]="hasDemographics() ? 'lucideCheck' : 'lucideAlertCircle'" size="12" [class.text-emerald-500]="hasDemographics()" [class.text-amber-500]="!hasDemographics()" />
            <span>Demographics</span>
          </button>
          <button (click)="activeTab.set('contact')" [class.bg-emerald-500\/10]="hasContact()" class="px-2.5 py-1 rounded-lg border border-border flex items-center gap-1.5 hover:bg-accent transition-colors">
            <ng-icon [name]="hasContact() ? 'lucideCheck' : 'lucideAlertCircle'" size="12" [class.text-emerald-500]="hasContact()" [class.text-amber-500]="!hasContact()" />
            <span>Contact & Emergency</span>
          </button>
          <button (click)="activeTab.set('insurance')" [class.bg-emerald-500\/10]="hasInsurance()" class="px-2.5 py-1 rounded-lg border border-border flex items-center gap-1.5 hover:bg-accent transition-colors">
            <ng-icon [name]="hasInsurance() ? 'lucideCheck' : 'lucideAlertCircle'" size="12" [class.text-emerald-500]="hasInsurance()" [class.text-amber-500]="!hasInsurance()" />
            <span>Insurance Info</span>
          </button>
          <button (click)="activeTab.set('allergies')" [class.bg-emerald-500\/10]="hasAllergies()" class="px-2.5 py-1 rounded-lg border border-border flex items-center gap-1.5 hover:bg-accent transition-colors">
            <ng-icon [name]="hasAllergies() ? 'lucideCheck' : 'lucideAlertCircle'" size="12" [class.text-emerald-500]="hasAllergies()" [class.text-amber-500]="!hasAllergies()" />
            <span>Allergies & Safety</span>
          </button>
          <button (click)="activeTab.set('history')" [class.bg-emerald-500\/10]="hasHistory()" class="px-2.5 py-1 rounded-lg border border-border flex items-center gap-1.5 hover:bg-accent transition-colors">
            <ng-icon [name]="hasHistory() ? 'lucideCheck' : 'lucideAlertCircle'" size="12" [class.text-emerald-500]="hasHistory()" [class.text-amber-500]="!hasHistory()" />
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="lg:col-span-8 space-y-6">
          ...
          <div class="w-full space-y-4">
            ...
            <div hlmTabsList class="w-full grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-1 bg-muted p-1.5 rounded-xl border border-border">
              <button
                hlmTabsTrigger
                type="button"
            ...
            <div *ngIf="activeTab() === 'demographics'" hlmTabsContent class="p-6 rounded-2xl border border-border bg-card space-y-5 shadow-xs">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                ...
                <div class="md:col-span-2 space-y-1">
                  <div class="flex justify-between items-center">
                    ...
                    <button *ngIf="isInitialSaveCompleted()" (click)="triggerLockedFieldNotice('Full Legal Name')" class="text-[10px] text-primary hover:underline flex items-center gap-1">
                  ...
                  <input
                    ...
                    (click)="isInitialSaveCompleted() ? triggerLockedFieldNotice('Full Legal Name') : null"
                  ...
                  <p class="text-[11px] text-muted-foreground">
                    Hint: Enter your name exactly as shown on your official photo ID (Aadhaar, Passport, Driver's License) for claim settlement.
                  </p>
                </div>
                ...
                <div class="space-y-1">
                  <div class="flex justify-between items-center">
                    <label class="font-semibold text-foreground">Ayushman Bharat Health Account (ABHA ID)</label>
                  ...
                  <input
                    ...
                    [disabled]="isInitialSaveCompleted()"
                    (click)="isInitialSaveCompleted() ? triggerLockedFieldNotice('ABHA Health ID') : null"
                  ...
                  <p class="text-[11px] text-muted-foreground">
                    Hint: 14-digit ABHA Number or ABDM Address used to link health records across national hospitals.
                  </p>
                </div>

                <div class="space-y-1">
                  <label class="font-semibold block text-foreground">Medical Record Number (MRN Code)</label>
                  <input
                    hlmInput
                    type="text"
                  ...
                  <p class="text-[11px] text-muted-foreground">
                    Hint: System-generated unique EHR identifier for inpatient and outpatient charts.
                  </p>
                </div>
                ...
                <div class="space-y-1">
                  ...
                  <input
                    ...
                    class="w-full h-9 text-xs disabled:opacity-75 disabled:cursor-not-allowed disabled:bg-muted/50" />
                  <p class="text-[11px] text-muted-foreground">
                    Hint: Essential for age-based clinical risk stratification and prescription dosage safety checks.
                  </p>
                </div>
                ...
                <div class="space-y-1">
                  ...
                  <select
                    ...
                    [disabled]="isInitialSaveCompleted()"
                    (change)="isInitialSaveCompleted() ? triggerLockedFieldNotice('Administrative Gender') : null"
                  ...
                  <p class="text-[11px] text-muted-foreground">
                    Hint: Used for gender-specific physiological reference ranges in diagnostic laboratory reports.
                  </p>
                </div>
                ...
                <div class="space-y-1 md:col-span-2">
                  ...
                  <select
                    ...
                    [disabled]="isInitialSaveCompleted()"
                    (change)="isInitialSaveCompleted() ? triggerLockedFieldNotice('Blood Type') : null"
                    class="w-full h-9 text-xs bg-background disabled:opacity-75 disabled:cursor-not-allowed disabled:bg-muted/50">
                  ...
                  <p class="text-[11px] text-muted-foreground">
                    Hint: Critical for emergency blood transfusion readiness and cross-matching during surgical procedures.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div *ngIf="activeTab() === 'contact'" hlmTabsContent class="p-6 rounded-2xl border border-border bg-card space-y-5 shadow-xs">
              <div class="flex items-center justify-between border-b border-border pb-3">
                <div>
                  <h2 class="text-sm font-bold text-foreground flex items-center gap-2">
                  ...
                  <p class="text-xs text-muted-foreground mt-0.5">Primary communication details and urgent hospital intake contact.</p>
                </div>
                <span hlmBadge variant="secondary" class="text-[10px] text-emerald-600 dark:text-emerald-400">Self-Editable</span>
              </div>
              ...
              <div class="space-y-5 text-xs">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div class="space-y-1">
                    <label class="font-semibold block text-foreground">Primary Mobile Phone *</label>
                    <input hlmInput type="text" [(ngModel)]="profileForm.phone" placeholder="+1 (555) 019-2834" class="w-full h-9 text-xs" />
                    <p class="text-[11px] text-muted-foreground">
                      Hint: Receives appointment confirmation SMS, virtual visit links, and lab notification alerts.
                    </p>
                  </div>

                  <div class="space-y-1">
                    <label class="font-semibold block text-foreground">Email Address *</label>
                    <input hlmInput type="email" [(ngModel)]="profileForm.email" placeholder="patient@example.com" class="w-full h-9 text-xs" />
                    <p class="text-[11px] text-muted-foreground">
                      Hint: Secure e-delivery of discharge summaries, consultation reports, and billing receipts.
                    </p>
                  </div>
                </div>

                <div class="space-y-1">
                  <label class="font-semibold block text-foreground">Residential Street Address *</label>
                  <input hlmInput type="text" [(ngModel)]="profileForm.address" placeholder="123 Health Science Way, Suite 400, Boston, MA 02115" class="w-full h-9 text-xs" />
                  <p class="text-[11px] text-muted-foreground">
                    Hint: Physical residence address required for pharmacy prescription delivery and home care services.
                  </p>
                </div>
                ...
                <div class="p-5 rounded-2xl bg-muted/30 border border-primary/20 space-y-4">
                  <div class="flex items-center justify-between border-b border-border pb-2.5">
                    <div class="flex items-center gap-2">
                      ...
                      <div>
                        <h3 class="font-bold text-foreground text-xs">Primary Emergency Contact & Next of Kin</h3>
                        <p class="text-[11px] text-muted-foreground">Required for emergency clinical triage and hospital admission.</p>
                      </div>
                    </div>
                    <span hlmBadge variant="outline" class="text-[10px] border-amber-500/30 text-amber-600 dark:text-amber-400">Clinical Intake</span>
                  </div>
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                    ...
                    <div class="space-y-1">
                      ...
                      <p class="text-[10px] text-muted-foreground">Hint: Full legal name of next of kin.</p>
                    </div>
                    ...
                    <div class="space-y-1">
                      ...
                      <p class="text-[10px] text-muted-foreground">Hint: Legal relationship to patient.</p>
                    </div>
                    ...
                    <div class="space-y-1">
                      ...
                      <input
                        ...
                        class="w-full h-9 text-xs bg-background" />
                      <p class="text-[10px] text-muted-foreground">Hint: 24/7 accessible phone number.</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div *ngIf="activeTab() === 'insurance'" hlmTabsContent class="p-6 rounded-2xl border border-border bg-card space-y-5 shadow-xs">
              <div class="flex items-center justify-between border-b border-border pb-3">
                <div>
                  <h2 class="text-sm font-bold text-foreground flex items-center gap-2">
                  ...
                  <p class="text-xs text-muted-foreground mt-0.5">Insurance carrier pre-authorization and policy details for medical claim filing.</p>
                </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                <div class="space-y-1">
                  <label class="font-semibold block text-foreground">Insurance Carrier / Provider Name *</label>
                  <input hlmInput type="text" [(ngModel)]="profileForm.insuranceProvider" placeholder="e.g. BlueCross BlueShield / Aetna / Self-Pay" class="w-full h-9 text-xs" />
                  <p class="text-[11px] text-muted-foreground">
                ...
                <div class="space-y-1">
                  ...
                  <select hlmInput [(ngModel)]="profileForm.coveragePlan" class="w-full h-9 text-xs bg-background">
                ...
                <div class="space-y-1">
                  <label class="font-semibold block text-foreground">Policy / Member ID Number *</label>
                  <input hlmInput type="text" [(ngModel)]="profileForm.insurancePolicyNumber" placeholder="e.g. BCBS-88912301" class="w-full h-9 text-xs font-mono" />
                  <p class="text-[11px] text-muted-foreground">
                ...
                <div class="space-y-1">
                  ...
                  <input hlmInput type="text" [(ngModel)]="profileForm.insuranceGroupNumber" placeholder="e.g. GRP-50192" class="w-full h-9 text-xs font-mono" />
                  <p class="text-[11px] text-muted-foreground">
            ...
            <div *ngIf="activeTab() === 'allergies'" hlmTabsContent class="p-6 rounded-2xl border border-border bg-card space-y-5 shadow-xs">
              <div class="flex items-center justify-between border-b border-border pb-3">
                <div>
                  <h2 class="text-sm font-bold text-foreground flex items-center gap-2">
                  ...
                  <p class="text-xs text-muted-foreground mt-0.5">Critical allergy alerts used by physicians and pharmacists prior to prescribing.</p>
                </div>
              ...
              <div class="space-y-4 text-xs">
                <div class="space-y-1">
                  <label class="font-bold block text-destructive flex items-center gap-2">
                    ...
                    <span hlmBadge variant="outline" class="text-[9px] border-destructive/40 text-destructive">Rx Engine Audited</span>
                  </label>
                  <input hlmInput type="text" [(ngModel)]="profileForm.foodAllergies" placeholder="Peanuts, Shellfish, Gluten, Penicillin, Amoxicillin, Latex..." class="w-full h-9 text-xs" />
                  <p class="text-[11px] text-muted-foreground">
                    Hint: List any drug hypersensitivities or severe food allergies (e.g. Penicillin, Peanuts). Automated clinical engines cross-check prescriptions against this list.
                  </p>
                </div>

                <div class="space-y-1">
                  <label class="font-semibold block text-foreground">Special Medical Alerts & Physical Contraindications</label>
                  <textarea hlmTextarea [(ngModel)]="profileForm.medicalAlerts" rows="3" placeholder="e.g. Cardiac Pacemaker, Latex Anaphylaxis, Fall Risk, Dialysis Patient..." class="w-full text-xs"></textarea>
                  <p class="text-[11px] text-muted-foreground">
                    Hint: Important medical devices or acute vulnerabilities (e.g. Pacemaker, Implantable Defibrillator, Seizure Disorder).
                  </p>
                </div>
              </div>
            </div>
            ...
            <div *ngIf="activeTab() === 'history'" hlmTabsContent class="p-6 rounded-2xl border border-border bg-card space-y-5 shadow-xs">
              <div class="flex items-center justify-between border-b border-border pb-3">
                <div>
                  <h2 class="text-sm font-bold text-foreground flex items-center gap-2">
                  ...
                  <p class="text-xs text-muted-foreground mt-0.5">Past medical history, surgical procedures, and social/lifestyle health indicators.</p>
                </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                <div class="md:col-span-2 space-y-1">
                  <label class="font-semibold block text-foreground">Previous Major Illnesses & Medical History</label>
                  <input hlmInput type="text" [(ngModel)]="profileForm.pastMedicalHistory" placeholder="e.g. Asthma, Childhood Pneumonia, Hypertension..." class="w-full h-9 text-xs" />
                  <p class="text-[11px] text-muted-foreground">
                    Hint: Document past acute conditions or chronic illnesses managed in previous health encounters.
                  </p>
                </div>

                <div class="md:col-span-2 space-y-1">
                  <label class="font-semibold block text-foreground">Serious Medical Conditions / Chronic Diseases</label>
                  <input hlmInput type="text" [(ngModel)]="profileForm.seriousConditions" placeholder="e.g. Type 2 Diabetes Mellitus, Coronary Artery Disease..." class="w-full h-9 text-xs" />
                  <p class="text-[11px] text-muted-foreground">
                    Hint: Ongoing long-term conditions requiring active clinical management and monitoring.
                  </p>
                </div>

                <div class="space-y-1">
                  <label class="font-semibold block text-foreground">Past Surgeries & Procedures</label>
                  <input hlmInput type="text" [(ngModel)]="profileForm.surgeriesAndProcedures" placeholder="e.g. Appendectomy (2018), Knee Arthroscopy (2021)..." class="w-full h-9 text-xs" />
                  <p class="text-[11px] text-muted-foreground">
                ...
                <div class="space-y-1">
                  ...
                  <input hlmInput type="text" [(ngModel)]="profileForm.familyMedicalHistory" placeholder="e.g. Maternal Hypertension, Paternal Diabetes..." class="w-full h-9 text-xs" />
                  <p class="text-[11px] text-muted-foreground">
                ...
                <div class="space-y-1">
                  ...
                  <p class="text-[11px] text-muted-foreground">Hint: Informs clinical nutrition and dietary care plans.</p>
                </div>

                <div class="space-y-1">
                  <label class="font-semibold block text-foreground">Smoking / Tobacco Status</label>
                  <select hlmInput [(ngModel)]="profileForm.smokingStatus" class="w-full h-9 text-xs bg-background">
                    <option value="Never">Never Smoked</option>
                    <option value="Former Smoker">Former Smoker</option>
                    <option value="Current Smoker">Current Smoker</option>
                  </select>
                  <p class="text-[11px] text-muted-foreground">Hint: Cardiovascular and respiratory health indicator.</p>
                </div>
                ...
                <div class="space-y-1">
                  ...
                  <p class="text-[11px] text-muted-foreground">Hint: Pharmacokinetic evaluation parameter.</p>
                </div>

                <div class="space-y-1">
                  <label class="font-semibold block text-foreground">Exercise Routine</label>
                  <select hlmInput [(ngModel)]="profileForm.exerciseRoutine" class="w-full h-9 text-xs bg-background">
                  ...
                  <p class="text-[11px] text-muted-foreground">Hint: Physical activity and wellness baseline.</p>
                </div>
              </div>
            </div>
          ...
          <div class="p-4 rounded-xl border border-border bg-card flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-xs">
            <div class="flex items-center gap-2">
        ...
        <div class="lg:col-span-4 space-y-6">
          ...
          <div class="p-5 rounded-2xl border border-border bg-card space-y-4 shadow-xs">
            <div class="flex items-center gap-3">
              ...
              <div class="space-y-0.5">
                <h3 class="font-bold text-foreground text-sm">{{ profileForm.fullName || patient()?.fullName }}</h3>
                <p class="text-xs text-muted-foreground">MRN: <span class="font-mono text-foreground font-medium">{{ patient()?.patientCode || 'PAT-1001' }}</span></p>
                <span hlmBadge [variant]="isInitialSaveCompleted() ? 'secondary' : 'outline'" class="text-[10px] mt-1">
            ...
            <div class="p-3.5 rounded-xl bg-muted/40 border border-border space-y-2 text-xs">
              <div class="flex justify-between items-center">
                ...
                <span class="font-mono text-foreground font-semibold text-[11px]">{{ profileForm.abhaId || profileForm.nationalId || 'N/A' }}</span>
              </div>
              <div class="flex justify-between items-center">
                <span class="text-muted-foreground">Date of Birth:</span>
                <span class="font-semibold text-foreground">{{ profileForm.dateOfBirth || 'N/A' }}</span>
              </div>
              <div class="flex justify-between items-center">
              ...
              <div class="flex justify-between items-center">
                ...
                <span class="font-bold font-mono text-primary">{{ profileForm.bloodType || 'N/A' }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="p-5 rounded-2xl border border-primary/20 bg-primary/5 space-y-4">
            ...
            <div class="space-y-2.5 pt-1 text-xs">
              <div class="flex items-center justify-between p-2 rounded-lg bg-background/60 border border-border">
                <span class="text-muted-foreground flex items-center gap-1.5">
                  <ng-icon [name]="hasDemographics() ? 'lucideCheck' : 'lucideAlertCircle'" size="14" [class.text-emerald-500]="hasDemographics()" [class.text-amber-500]="!hasDemographics()" />
                  Legal Identity & ABHA ID
                </span>
                <span class="font-medium text-[11px]" [class.text-emerald-600]="hasDemographics()" [class.text-amber-600]="!hasDemographics()">
              ...
              <div class="flex items-center justify-between p-2 rounded-lg bg-background/60 border border-border">
                <span class="text-muted-foreground flex items-center gap-1.5">
                  <ng-icon [name]="hasContact() ? 'lucideCheck' : 'lucideAlertCircle'" size="14" [class.text-emerald-500]="hasContact()" [class.text-amber-500]="!hasContact()" />
                  Contact & Emergency Phone
                </span>
                <span class="font-medium text-[11px]" [class.text-emerald-600]="hasContact()" [class.text-amber-600]="!hasContact()">
              ...
              <div class="flex items-center justify-between p-2 rounded-lg bg-background/60 border border-border">
                <span class="text-muted-foreground flex items-center gap-1.5">
                  <ng-icon [name]="hasInsurance() ? 'lucideCheck' : 'lucideAlertCircle'" size="14" [class.text-emerald-500]="hasInsurance()" [class.text-amber-500]="!hasInsurance()" />
                  Insurance Carrier Details
                </span>
                <span class="font-medium text-[11px]" [class.text-emerald-600]="hasInsurance()" [class.text-amber-600]="!hasInsurance()">
              ...
              <div class="flex items-center justify-between p-2 rounded-lg bg-background/60 border border-border">
                <span class="text-muted-foreground flex items-center gap-1.5">
                  <ng-icon [name]="hasAllergies() ? 'lucideCheck' : 'lucideAlertCircle'" size="14" [class.text-emerald-500]="hasAllergies()" [class.text-amber-500]="!hasAllergies()" />
                  Allergies & Contraindications
                </span>
                <span class="font-medium text-[11px]" [class.text-emerald-600]="hasAllergies()" [class.text-amber-600]="!hasAllergies()">
              ...
              <div class="flex items-center justify-between p-2 rounded-lg bg-background/60 border border-border">
                <span class="text-muted-foreground flex items-center gap-1.5">
                  <ng-icon [name]="hasHistory() ? 'lucideCheck' : 'lucideAlertCircle'" size="14" [class.text-emerald-500]="hasHistory()" [class.text-amber-500]="!hasHistory()" />
                  Clinical History & Lifestyle
                </span>
                <span class="font-medium text-[11px]" [class.text-emerald-600]="hasHistory()" [class.text-amber-600]="!hasHistory()">
          ...
          <div class="p-5 rounded-2xl border border-border bg-card space-y-3 shadow-xs text-xs">
            ...
            <div class="space-y-2">
              ...
              <div class="space-y-0.5 pt-2 border-t border-border">
                ...
                <span class="font-semibold text-foreground block">{{ profileForm.insuranceProvider || 'Self-Pay' }}</span>
                <span *ngIf="profileForm.insurancePolicyNumber" class="font-mono text-[11px] text-muted-foreground block">Policy: {{ profileForm.insurancePolicyNumber }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="p-4 rounded-2xl border border-amber-500/30 bg-amber-500/5 space-y-2 text-xs">
            ...
            <button (click)="triggerLockedFieldNotice('Legal Identity')" class="text-xs text-primary font-medium hover:underline inline-flex items-center gap-1 pt-1">
      ...
      <div *ngIf="showLockedModal()" class="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in duration-200">
        <div class="bg-card border border-border rounded-2xl max-w-md w-full p-6 space-y-5 shadow-2xl relative">
          ...
          <div class="space-y-2">
            <span class="text-[11px] font-semibold text-foreground block">Need to update your legal identity or ABHA health record?</span>
