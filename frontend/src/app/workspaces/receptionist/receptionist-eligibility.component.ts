...
import {
  ...
  CopayCollectionDTO,
} from '../../core/models/billing-eligibility.model';
    ...
    <div
      [ngClass]="
        isModal
          ...
          : 'space-y-6 max-w-5xl mx-auto pb-12'
      "
    >
      <div
        [ngClass]="
          isModal
            ? 'w-full max-w-4xl max-h-[90vh] overflow-y-auto p-6 space-y-6 border border-border shadow-2xl bg-card rounded-2xl'
            : 'space-y-6 bg-card p-6 rounded-2xl border border-border shadow-sm'
        "
      >
        <!-- Simple Header -->
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-xs text-muted-foreground">
                Verify active coverage, check copays & collect front-desk payments
              </p>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
            <div class="sm:col-span-2 space-y-1">
              <label class="font-medium text-foreground flex items-center gap-1">
                <ng-icon name="lucideUser" size="14" class="text-purple-600" /> Patient Directory
                Search
              </label>
              <select
                [(ngModel)]="patientId"
              ...
              >
                ...
                <option *ngFor="let p of allPatients()" [value]="p.id">
                  #{{ p.id }} - {{ p.fullName }} | Ins: {{ p.insuranceProvider || 'N/A' }} ({{
                    p.insurancePolicyNumber || 'No Policy #'
                  }})
                </option>
              </select>
            </div>
            ...
            <div class="space-y-1">
              ...
              <div class="flex flex-wrap gap-1">
                <button
                  *ngFor="let preset of payerPresets"
                  type="button"
                  (click)="applyPreset(preset)"
                  class="px-2 py-1 text-[11px] rounded bg-muted/60 hover:bg-purple-500/10 hover:text-purple-600 border border-border transition-colors"
                >
                  {{ preset.name }}
                </button>
              </div>
          ...
          <form (ngSubmit)="onRunRTE()" class="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs pt-1">
            ...
            <div class="space-y-1">
              ...
              <input
                ...
                [(ngModel)]="payerName"
                name="payerName"
                placeholder="Star Health / PM-JAY"
            ...
            <div class="sm:col-span-3 flex justify-end pt-1">
              <button
                ...
                type="submit"
                [disabled]="loading()"
                class="text-xs bg-purple-600 hover:bg-purple-700 text-white font-bold gap-2"
              >
                <ng-icon *ngIf="!loading()" name="lucideSend" size="14" />
                <ng-icon *ngIf="loading()" name="lucideRefreshCw" size="14" class="animate-spin" />
                <span>{{
                  loading() ? 'Verifying Coverage...' : 'Check Insurance Eligibility'
                }}</span>
              </button>
            </div>
          </form>
        ...
        <div *ngIf="rteResult()" class="space-y-4 pt-4 border-t border-border">
          <!-- Status Banner -->
          <div
            class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30"
          >
            <div class="space-y-1">
              <div class="flex items-center gap-2">
                <span
                  hlmBadge
                  variant="default"
                  class="text-xs bg-emerald-600 text-white font-bold gap-1"
                >
              ...
              <p class="text-xs text-muted-foreground font-mono">
                Control #: {{ rteResult()?.transactionControlNumber }} | Subscriber:
                {{ subscriberId }}
              </p>
            </div>
            ...
            <button
              ...
              variant="default"
              size="sm"
              (click)="openCopayModal()"
              class="text-xs bg-emerald-600 hover:bg-emerald-700 text-white font-bold gap-1.5 shadow-sm"
            >
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
            ...
            <div class="p-3.5 rounded-xl bg-purple-500/5 border border-purple-500/20 space-y-2">
              ...
              <div class="space-y-1 font-mono text-[11px]">
                <div class="flex justify-between">
                  <span>Primary Care (PCP):</span> <strong>₹500.00</strong>
                </div>
                <div class="flex justify-between">
                  <span>Specialist Visit:</span> <strong>₹1,000.00</strong>
                </div>
                <div class="flex justify-between">
                  <span>Urgent Care:</span> <strong>₹1,500.00</strong>
                </div>
                <div class="flex justify-between">
                  <span>Emergency Room:</span> <strong>₹3,000.00</strong>
                </div>
              </div>
            </div>
            ...
            <div class="p-3.5 rounded-xl bg-sky-500/5 border border-sky-500/20 space-y-2">
              ...
              <div class="space-y-1 font-mono text-[11px]">
                <div class="flex justify-between">
                  <span>Total Deductible:</span> <strong>₹15,000.00</strong>
                </div>
                <div class="flex justify-between">
                  <span>Deductible Met:</span> <strong class="text-emerald-600">₹5,000.00</strong>
                </div>
                <div class="flex justify-between">
                  <span>Remaining:</span> <strong class="text-amber-600">₹10,000.00</strong>
                </div>
            ...
            <div class="p-3.5 rounded-xl bg-emerald-500/5 border border-emerald-500/20 space-y-2">
              ...
              <div class="space-y-1 font-mono text-[11px]">
                <div class="flex justify-between">
                  <span>Coverage Ratio:</span> <strong>80% / 20%</strong>
                </div>
                <div class="flex justify-between">
                  <span>OOP Max Total:</span> <strong>₹50,000.00</strong>
                </div>
                <div class="flex justify-between">
                  <span>OOP Met YTD:</span> <strong class="text-emerald-600">₹21,000.00</strong>
                </div>
              </div>
            </div>
          </div>
          ...
          <div *ngIf="rteResult()?.coverageAlerts?.length" class="space-y-1.5">
            ...
            <div class="space-y-1 text-xs">
              <div
                *ngFor="let alert of rteResult()?.coverageAlerts"
                class="p-2 rounded bg-purple-500/10 text-purple-700 dark:text-purple-300 border border-purple-500/20 flex items-center gap-2"
              >
          ...
          <div hlmCard class="w-full max-w-md p-5 space-y-4 border border-border shadow-lg">
            <div class="flex items-center justify-between pb-2 border-b border-border">
              ...
              <button
                ...
                (click)="showCopayModal.set(false)"
              >
                &times;
              </button>
            </div>
            ...
            <div *ngIf="!receiptResult()" class="space-y-3 text-xs">
              <div class="space-y-1">
                ...
                <input
                  ...
                  type="number"
                  [(ngModel)]="copayAmount"
                  class="w-full font-bold text-emerald-600 text-sm"
                />
              </div>
              ...
              <div class="flex justify-end gap-2 pt-2">
                ...
                <button
                  ...
                  (click)="submitCopay()"
                  [disabled]="collecting()"
                  class="text-xs bg-emerald-600 hover:bg-emerald-700 text-white font-bold"
                >
            ...
            <div
              *ngIf="receiptResult()"
              class="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-xs space-y-2"
            >
              ...
              <div class="font-mono space-y-1 text-foreground">
                <div>
                  Receipt #: <strong>{{ receiptResult()?.receiptNumber }}</strong>
                </div>
                <div>
                  Amount Paid:
                  <strong>₹{{ receiptResult()?.amountCollected | number: '1.2-2' }}</strong>
                </div>
                <div>
                  Payment Method: <strong>{{ receiptResult()?.paymentMethod }}</strong>
                </div>
                <div>
                  Collected By: <strong>{{ receiptResult()?.collectedBy }}</strong>
                </div>
              </div>
              <div class="flex justify-between items-center pt-2 border-t border-emerald-500/20">
                ...
                <button
                  ...
                  variant="default"
                  size="sm"
                  (click)="showCopayModal.set(false); receiptResult.set(null)"
                  class="text-xs bg-purple-600"
                >
                  Done
                </button>
              </div>
