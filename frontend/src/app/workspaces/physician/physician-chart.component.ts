...
} from '../../core/models/imaging.model';
import {
  ProcedureOrder,
  ...
  template: `
    <div class="space-y-6">
      ...
      <div class="rounded-xl border border-border bg-card p-5 shadow-xs space-y-4">
        ...
        >
          <div>
            ...
            <p class="text-xs text-muted-foreground mt-0.5">
              Comprehensive physician chart for SOAP notes, ICD-10 diagnoses, eRx orders, imaging
              PACS, surgical procedures, care teams, documents, and informed consents.
            </p>
          </div>
          ...
          <div class="flex items-center gap-3 w-full md:w-auto">
            <div
              class="flex items-center gap-2 bg-muted/30 border border-border rounded-lg p-1.5 w-full md:w-auto"
            >
              ...
              >
                <option *ngIf="patients().length === 0" [value]="null">
                  Loading MPI census...
                </option>
        ...
        <div
          *ngIf="activePatient() as patient; else noPatientSelected"
          class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3 text-xs bg-muted/20 p-3.5 rounded-lg border border-border/60"
        >
          <div>
            <span
          ...
          <div>
            <span
              ...
              >MRN Code</span
            >
            <span class="font-mono font-medium text-foreground block">{{
              patient.patientCode
            }}</span>
          </div>
          ...
          <div>
            ...
            <span class="text-foreground block"
              >{{ patient.dateOfBirth || 'N/A' }} ({{ patient.gender || 'U' }})</span
            >
          </div>
          ...
          <div>
            ...
            <span
              hlmBadge
              variant="outline"
              class="text-[10px] font-semibold text-primary border-primary/30 bg-primary/10"
            >
              {{ patient.bloodType || 'A+' }}
            </span>
          </div>
          ...
          <div>
            <span
              class="text-[10px] text-muted-foreground uppercase tracking-wider block font-semibold"
              >Clinical Status</span
            >
            <span
              hlmBadge
              variant="secondary"
              class="text-[10px] bg-emerald-500/10 text-emerald-600"
              >Active Chart Context</span
            >
          </div>
        </div>
        ...
        <ng-template #noPatientSelected>
          <div
            class="p-4 bg-amber-500/10 border border-amber-500/20 rounded-lg text-amber-700 dark:text-amber-300 text-xs flex items-center justify-between"
          >
            <span class="flex items-center gap-2">
              <ng-icon name="lucideTriangleAlert" size="16" /> No active patient selected. Please
              select a patient from the dropdown or MPI census.
            </span>
          </div>
        </ng-template>
        ...
        <div
          class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-1.5 p-1.5 bg-muted/40 rounded-xl border border-border"
        >
          <button
      ...
      <div *ngIf="activeTab() === 'diagnoses'" class="space-y-6">
        ...
        >
          <div>
            ...
            <p class="text-xs text-muted-foreground mt-0.5">
              Document clinical diagnoses, problem severity, and ICD-10/SNOMED terminology codes
              with intelligent lookup.
            </p>
          </div>
          <button
            hlmBtn
            variant="default"
            size="sm"
            (click)="showDiagnosisModal.set(true)"
        ...
        <div class="rounded-xl border border-border bg-card overflow-hidden shadow-xs">
          <div class="p-4 border-b border-border bg-muted/20 flex justify-between items-center">
            ...
            <span class="text-[11px] text-muted-foreground"
              >{{ diagnoses().length }} active conditions</span
            >
          </div>
              ...
              <tbody hlmTableBody class="divide-y divide-border">
                <tr
                  *ngFor="let d of diagnoses()"
                  ...
                  class="hover:bg-muted/40 transition-colors"
                >
                  <td hlmTableCell class="py-3 px-4 font-mono font-bold">
                    <span hlmBadge variant="outline">{{ d.icdCode || 'N/A' }}</span>
                  </td>
                  <td hlmTableCell class="py-3 px-4 font-semibold text-foreground">
                    {{ d.conditionName }}
                  </td>
                  <td hlmTableCell class="py-3 px-4">
                    <span hlmBadge variant="outline" class="text-[10px] text-emerald-600">{{
                      d.status || 'ACTIVE'
                    }}</span>
                  </td>
                </tr>
                <tr *ngIf="diagnoses().length === 0" hlmTableRow>
                  <td
                    ...
                    class="py-12 text-center text-muted-foreground text-xs"
                  >
                    No active ICD-10 diagnoses documented for this patient.
                  </td>
                </tr>
              </tbody>
            </table>
      ...
      <div *ngIf="activeTab() === 'erx'" class="space-y-6">
        ...
        >
          <div>
            ...
            <p class="text-xs text-muted-foreground mt-0.5">
              Issue eRx medication orders with real-time allergy/interaction safety validation.
            </p>
          </div>
          <button
            hlmBtn
            variant="default"
            size="sm"
            (click)="showErxModal.set(true)"
            class="gap-1.5 font-semibold text-xs bg-emerald-600 hover:bg-emerald-700 text-white"
          >
        ...
        <div class="rounded-xl border border-border bg-card overflow-hidden shadow-xs">
          <div class="p-4 border-b border-border bg-muted/20 flex justify-between items-center">
            ...
            <span class="text-[11px] text-muted-foreground"
              >{{ prescriptions().length }} active orders</span
            >
          </div>
              ...
              <tbody hlmTableBody class="divide-y divide-border">
                ...
                <tr *ngIf="prescriptions().length === 0" hlmTableRow>
                  <td
                    ...
                    class="py-12 text-center text-muted-foreground text-xs"
                  >
                    No active eRx orders logged for this patient.
                  </td>
                </tr>
              </tbody>
            </table>
      ...
      <div *ngIf="activeTab() === 'allergies'" class="space-y-6">
        ...
        >
          <div>
            ...
            <p class="text-xs text-muted-foreground mt-0.5">
              Review allergen safety records, severity, and adverse reaction history.
            </p>
          </div>
          <button
            hlmBtn
            ...
            class="gap-1.5 font-semibold text-xs bg-amber-600 hover:bg-amber-700 text-white"
          >
        ...
        <div class="rounded-xl border border-border bg-card overflow-hidden shadow-xs">
          <div class="p-4 border-b border-border bg-muted/20 flex justify-between items-center">
            ...
            <span class="text-[11px] text-muted-foreground"
              >{{ allergies().length }} documented</span
            >
          </div>
            ...
            <table hlmTable class="w-full text-xs">
              ...
              <tbody hlmTableBody class="divide-y divide-border">
                ...
                >
                  ...
                  <td hlmTableCell class="py-3 px-4 text-muted-foreground">
                    {{ a.reactionDescription || 'None detailed' }}
                  </td>
                  <td hlmTableCell class="py-3 px-4">
                    <span hlmBadge variant="outline" class="text-[10px]">{{ a.status }}</span>
                  </td>
                </tr>
                <tr *ngIf="allergies().length === 0" hlmTableRow>
                  <td
                    ...
                    class="py-12 text-center text-muted-foreground text-xs"
                  >
                    No active allergies documented for this patient.
                  </td>
                </tr>
              </tbody>
            </table>
      ...
      <div *ngIf="activeTab() === 'vitals'" class="space-y-6">
        ...
        >
          <div>
            ...
            <p class="text-xs text-muted-foreground mt-0.5">
              Monitor physiological vital signs, blood pressure, pulse, temperature, and BMI.
            </p>
          </div>
          ...
          <button
            ...
            variant="default"
            size="sm"
            (click)="showVitalsModal.set(true)"
            class="gap-1.5 font-semibold text-xs bg-blue-600 hover:bg-blue-700 text-white"
          >
        ...
        <div class="rounded-xl border border-border bg-card overflow-hidden shadow-xs">
          <div class="p-4 border-b border-border bg-muted/20 flex justify-between items-center">
            ...
            <span class="text-[11px] text-muted-foreground"
              >{{ vitals().length }} readings recorded</span
            >
          </div>
            ...
            <table hlmTable class="w-full text-xs">
              ...
              <tbody hlmTableBody class="divide-y divide-border">
                ...
                >
                  ...
                  <td hlmTableCell class="py-3 px-4 font-semibold text-foreground font-mono">
      ...
      <div *ngIf="activeTab() === 'imaging'" class="space-y-6">
        ...
        >
          <div>
            ...
            <p class="text-xs text-muted-foreground mt-0.5">
              Order and review CT, MRI, X-Ray, Ultrasound studies, DICOM series, and radiologist
              reports.
            </p>
          </div>
          ...
          <button
            ...
            variant="default"
            size="sm"
            (click)="showImagingModal.set(true)"
            class="gap-1.5 font-semibold text-xs bg-indigo-600 hover:bg-indigo-700 text-white"
          >
        ...
        <div class="rounded-xl border border-border bg-card overflow-hidden shadow-xs">
          <div class="p-4 border-b border-border bg-muted/20 flex justify-between items-center">
            ...
            <span class="text-[11px] text-muted-foreground"
              >{{ imagingOrders().length }} orders</span
            >
          </div>
          ...
          <div class="overflow-x-auto">
            <table hlmTable class="w-full text-xs">
              ...
              <tbody hlmTableBody class="divide-y divide-border">
                <tr
                  *ngFor="let ord of imagingOrders()"
                  hlmTableRow
                  class="hover:bg-muted/40 transition-colors"
                >
                  <td hlmTableCell class="py-3 px-4">
                    <span
                      hlmBadge
                      variant="outline"
                      class="font-mono font-bold bg-indigo-500/10 text-indigo-600 border-indigo-200"
                      >{{ ord.modality }}</span
                    >
                  </td>
                  <td hlmTableCell class="py-3 px-4 font-semibold text-foreground">
                  ...
                  <td hlmTableCell class="py-3 px-4 text-muted-foreground">
                    {{ ord.orderedAt | date: 'short' }}
                  </td>
                  <td hlmTableCell class="py-3 px-4">
                    <span
                      hlmBadge
                      [variant]="ord.status === 'COMPLETED' ? 'secondary' : 'default'"
                      class="text-[10px]"
                      >{{ ord.status }}</span
                    >
                  </td>
                ...
                <tr *ngIf="imagingOrders().length === 0" hlmTableRow>
                  <td
                    ...
                    class="py-12 text-center text-muted-foreground text-xs"
                  >
                    No imaging orders recorded for this patient.
                  </td>
                </tr>
              </tbody>
            </table>
      ...
      <div *ngIf="activeTab() === 'procedures'" class="space-y-6">
        ...
        >
          <div>
            ...
            <p class="text-xs text-muted-foreground mt-0.5">
              Order clinical procedures, record surgical performance, and document operative notes.
            </p>
          </div>
          ...
          <button
            ...
            variant="default"
            size="sm"
            (click)="showProcedureModal.set(true)"
            class="gap-1.5 font-semibold text-xs bg-rose-600 hover:bg-rose-700 text-white"
          >
        ...
        <div class="rounded-xl border border-border bg-card overflow-hidden shadow-xs">
          <div class="p-4 border-b border-border bg-muted/20 flex justify-between items-center">
            ...
            <span class="text-[11px] text-muted-foreground"
              >{{ procedureOrders().length }} procedures</span
            >
          </div>
          ...
          <div class="overflow-x-auto">
            <table hlmTable class="w-full text-xs">
              ...
              <tbody hlmTableBody class="divide-y divide-border">
                ...
                >
                  <td hlmTableCell class="py-3 px-4 font-mono font-bold">
                    <span hlmBadge variant="outline">{{ p.procedureCode }}</span>
                  </td>
                  <td hlmTableCell class="py-3 px-4 font-semibold text-foreground">
                    {{ p.procedureName }}
                  </td>
                  ...
                  <td hlmTableCell class="py-3 px-4">
                    <span
                      ...
                      class="text-[10px]"
                      >{{ p.priority || 'ROUTINE' }}</span
                    >
                  </td>
                  <td hlmTableCell class="py-3 px-4">
                    <span hlmBadge variant="outline" class="text-[10px]">{{ p.status }}</span>
                  </td>
                </tr>
      ...
      <div *ngIf="activeTab() === 'care-team'" class="space-y-6">
        ...
        >
          <div>
            ...
            <p class="text-xs text-muted-foreground mt-0.5">
              Assign and coordinate primary attending physicians, consulting specialists, and
              primary care nurses.
            </p>
          </div>
          ...
          <button
            ...
            variant="default"
            size="sm"
            (click)="showCareTeamModal.set(true)"
            class="gap-1.5 font-semibold text-xs bg-cyan-600 hover:bg-cyan-700 text-white"
          >
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div
            *ngFor="let m of careTeamMembers()"
            class="rounded-xl border border-border bg-card p-4 shadow-xs space-y-3"
          >
            <div class="flex items-start justify-between">
              <div class="flex items-center gap-3">
                <div
                  class="size-10 rounded-full bg-cyan-500/10 text-cyan-600 flex items-center justify-center font-bold text-sm"
                >
            ...
            <div
              class="text-[11px] text-muted-foreground pt-2 border-t border-border flex justify-between items-center"
            >
          ...
          <div
            *ngIf="careTeamMembers().length === 0"
            class="col-span-3 rounded-xl border border-border bg-card p-12 text-center text-muted-foreground text-xs"
          >
      ...
      <div *ngIf="activeTab() === 'documents'" class="space-y-6">
        ...
        >
          <div>
            ...
            <p class="text-xs text-muted-foreground mt-0.5">
              Author and digitally finalize consultation notes, H&P, and discharge summaries with
              audit versioning.
            </p>
          </div>
        ...
        <div class="rounded-xl border border-border bg-card overflow-hidden shadow-xs">
          <div class="p-4 border-b border-border bg-muted/20 flex justify-between items-center">
            ...
            <span class="text-[11px] text-muted-foreground"
              >{{ clinicalDocuments().length }} documents</span
            >
          </div>
          ...
          <div class="overflow-x-auto">
            <table hlmTable class="w-full text-xs">
              ...
              <tbody hlmTableBody class="divide-y divide-border">
                ...
                >
                  <td hlmTableCell class="py-3 px-4">
                    <span hlmBadge variant="outline">{{ doc.documentType }}</span>
                  </td>
                  <td hlmTableCell class="py-3 px-4 font-semibold text-foreground">
                    {{ doc.title }}
                  </td>
                ...
                <tr *ngIf="clinicalDocuments().length === 0" hlmTableRow>
                  <td
                    ...
                    class="py-12 text-center text-muted-foreground text-xs"
                  >
                    No clinical documents authored yet.
                  </td>
                </tr>
              </tbody>
            </table>
      ...
      <div *ngIf="activeTab() === 'consents'" class="space-y-6">
        ...
        >
          <div>
            ...
            <p class="text-xs text-muted-foreground mt-0.5">
              Manage treatment consents, procedure agreements, ABDM health data sharing, and
              revocations.
            </p>
          </div>
          ...
          <button
            ...
            variant="default"
            size="sm"
            (click)="showConsentModal.set(true)"
            class="gap-1.5 font-semibold text-xs bg-purple-600 hover:bg-purple-700 text-white"
          >
        ...
        <div class="rounded-xl border border-border bg-card overflow-hidden shadow-xs">
          <div class="p-4 border-b border-border bg-muted/20 flex justify-between items-center">
            ...
            <span class="text-[11px] text-muted-foreground"
              >{{ patientConsents().length }} records</span
            >
          </div>
            ...
            <table hlmTable class="w-full text-xs">
              ...
              <tbody hlmTableBody class="divide-y divide-border">
                ...
                >
                  ...
                  <td hlmTableCell class="py-3 px-4 font-semibold text-foreground">
                    {{ c.consentTypeName || 'General Clinical Treatment' }}
                  </td>
                  <td hlmTableCell class="py-3 px-4 text-muted-foreground">
                    {{ c.signedByPatientName || activePatient()?.fullName }}
                  </td>
                  <td hlmTableCell class="py-3 px-4 text-muted-foreground">
                    {{ c.validFrom | date: 'shortDate' }}
                  </td>
                  <td hlmTableCell class="py-3 px-4">
                    <span
                      hlmBadge
                ...
                <tr *ngIf="patientConsents().length === 0" hlmTableRow>
                  <td
                    ...
                    class="py-12 text-center text-muted-foreground text-xs"
                  >
                    No active consent agreements documented.
                  </td>
                </tr>
              </tbody>
            </table>
      ...
      <div
        *ngIf="showDiagnosisModal()"
        class="fixed inset-0 z-50 bg-background/80 backdrop-blur-xs flex items-center justify-center p-4"
      >
        <div
          class="w-full max-w-md rounded-xl border border-border bg-card p-6 shadow-lg space-y-5"
        >
          <div class="flex justify-between items-center border-b border-border pb-3">
            ...
            <button
              ...
              variant="ghost"
              size="sm"
              (click)="showDiagnosisModal.set(false)"
              class="size-7 p-0"
            >
          ...
          <div class="space-y-3 text-xs">
            <div>
              <label class="font-medium text-foreground block mb-1"
                >Search Terminology / ICD-10</label
              >
              <div class="flex gap-2">
                <input
                  type="text"
                  [(ngModel)]="termSearchQuery"
                  (keyup.enter)="searchMedicalCodes()"
                  placeholder="Search condition name or code..."
              ...
              <div
                *ngIf="termResults().length > 0"
                class="mt-2 max-h-32 overflow-y-auto border border-border rounded-md divide-y divide-border bg-muted/30"
              >
                <div
                  *ngFor="let r of termResults()"
                  (click)="selectTerm(r)"
                  class="p-2 hover:bg-primary/10 cursor-pointer text-[11px] flex justify-between"
                >
            ...
            <div>
              ...
              <input
                type="text"
                [(ngModel)]="newDiagnosis.conditionName"
          ...
          <div class="flex justify-end gap-2 pt-2 border-t border-border">
            <button hlmBtn variant="outline" size="sm" (click)="showDiagnosisModal.set(false)">
              Cancel
            </button>
            <button
              hlmBtn
              variant="default"
              size="sm"
              [disabled]="savingDiagnosis() || !newDiagnosis.icdCode || !newDiagnosis.conditionName"
              (click)="saveDiagnosis()"
            >
      ...
      >
        <div
          class="w-full max-w-lg rounded-xl border border-border bg-card p-6 shadow-lg space-y-5"
        >
          <div class="flex justify-between items-center border-b border-border pb-3">
            <h3 class="text-sm font-bold text-foreground flex items-center gap-2">
          ...
          <div class="grid grid-cols-2 gap-3 text-xs">
            ...
            <div class="col-span-2">
              ...
              <input
                type="text"
                [(ngModel)]="newErx.frequency"
                placeholder="e.g. Twice daily with meals (BID)"
                class="w-full p-2 rounded-md border border-input bg-background"
              />
            </div>
          ...
          <div class="flex justify-end gap-2 pt-2 border-t border-border">
            <button hlmBtn variant="outline" size="sm" (click)="showErxModal.set(false)">
              Cancel
            </button>
            <button
              hlmBtn
              variant="default"
              size="sm"
              [disabled]="savingErx() || !newErx.medicationName || !newErx.dosage"
              (click)="saveErx()"
              class="bg-emerald-600 hover:bg-emerald-700 text-white"
            >
              <ng-icon name="lucideSave" size="14" class="mr-1" />
              {{ savingErx() ? 'Submitting...' : 'Issue eRx Order' }}
            </button>
          </div>
        </div>
      </div>
        ...
        >
          <div class="flex justify-between items-center border-b border-border pb-3">
            ...
            <button
              ...
              variant="ghost"
              size="sm"
              (click)="showAllergyModal.set(false)"
              class="size-7 p-0"
            >
              <ng-icon name="lucideX" size="16" />
            </button>
          ...
          <div class="space-y-3 text-xs">
            <div>
              ...
              <input
                type="text"
                [(ngModel)]="newAllergy.allergenName"
            ...
            <div>
              ...
              <textarea
                [(ngModel)]="newAllergy.reactionDescription"
          ...
          <div class="flex justify-end gap-2 pt-2 border-t border-border">
            <button hlmBtn variant="outline" size="sm" (click)="showAllergyModal.set(false)">
              Cancel
            </button>
            <button
              hlmBtn
              variant="default"
              size="sm"
              [disabled]="savingAllergy() || !newAllergy.allergenName"
              (click)="saveAllergy()"
      ...
      >
        <div
          class="w-full max-w-lg rounded-xl border border-border bg-card p-6 shadow-lg space-y-4"
        >
          <div class="flex justify-between items-center border-b border-border pb-3">
            <h3 class="text-sm font-bold text-foreground flex items-center gap-2">
            ...
            <button
              ...
              variant="ghost"
              size="sm"
              (click)="showVitalsModal.set(false)"
              class="size-7 p-0"
            >
              <ng-icon name="lucideX" size="16" />
            </button>
          ...
          <div class="flex justify-end gap-2 pt-2 border-t border-border">
            <button hlmBtn variant="outline" size="sm" (click)="showVitalsModal.set(false)">
              Cancel
            </button>
            <button
              hlmBtn
            ...
            >
              <ng-icon name="lucideSave" size="14" class="mr-1" />
              {{ savingVitals() ? 'Saving...' : 'Save Vitals Entry' }}
            </button>
          </div>
        </div>
      </div>
        ...
        >
          <div class="flex justify-between items-center border-b border-border pb-3">
            ...
            <button
              ...
              variant="ghost"
              size="sm"
              (click)="showImagingModal.set(false)"
              class="size-7 p-0"
            >
              <ng-icon name="lucideX" size="16" />
            </button>
          ...
          <div class="space-y-3 text-xs">
            ...
            <div>
              ...
              <input
                type="text"
                [(ngModel)]="newImaging.procedureName"
                placeholder="e.g. Chest 2-View, Brain with Contrast"
                class="w-full p-2 rounded-md border border-input bg-background"
              />
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                [(ngModel)]="newImaging.cptCode"
                placeholder="e.g. 71046, 70553"
                class="w-full p-2 rounded-md border border-input bg-background font-mono"
              />
            </div>
            ...
            <div>
              <label class="font-medium text-foreground block mb-1"
                >Clinical Indication / Notes</label
              >
              <textarea
                [(ngModel)]="newImaging.clinicalIndication"
                placeholder="e.g. Rule out pneumonia or effusion..."
                class="w-full p-2 rounded-md border border-input bg-background h-16"
              ></textarea>
            </div>
          </div>
          ...
          <div class="flex justify-end gap-2 pt-2 border-t border-border">
            <button hlmBtn variant="outline" size="sm" (click)="showImagingModal.set(false)">
              Cancel
            </button>
            <button
              hlmBtn
              variant="default"
              size="sm"
              [disabled]="savingImaging() || !newImaging.procedureName"
              (click)="saveImagingOrder()"
              class="bg-indigo-600 hover:bg-indigo-700 text-white"
            >
              <ng-icon name="lucideSave" size="14" class="mr-1" />
              {{ savingImaging() ? 'Submitting...' : 'Submit Imaging Order' }}
            </button>
          </div>
        </div>
      </div>
        ...
        >
          <div class="flex justify-between items-center border-b border-border pb-3">
            ...
            <button
              ...
              variant="ghost"
              size="sm"
              (click)="showProcedureModal.set(false)"
              class="size-7 p-0"
            >
              <ng-icon name="lucideX" size="16" />
            </button>
          ...
          <div class="space-y-3 text-xs">
            <div>
              <label class="font-medium text-foreground block mb-1"
                >Procedure Code (CPT/SNOMED) *</label
              >
              <input
                type="text"
                [(ngModel)]="newProcedure.procedureCode"
                placeholder="e.g. 47562, 43239"
                class="w-full p-2 rounded-md border border-input bg-background font-mono"
              />
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                [(ngModel)]="newProcedure.procedureName"
                placeholder="e.g. Laparoscopic Cholecystectomy, Diagnostic EGD"
                class="w-full p-2 rounded-md border border-input bg-background"
              />
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                [(ngModel)]="newProcedure.bodySite"
                placeholder="e.g. Right Upper Quadrant, Gastrointestinal"
                class="w-full p-2 rounded-md border border-input bg-background"
              />
            </div>
          ...
          <div class="flex justify-end gap-2 pt-2 border-t border-border">
            <button hlmBtn variant="outline" size="sm" (click)="showProcedureModal.set(false)">
              Cancel
            </button>
            <button
              hlmBtn
              variant="default"
              size="sm"
              [disabled]="savingProcedure() || !newProcedure.procedureName"
              (click)="saveProcedureOrder()"
              class="bg-rose-600 hover:bg-rose-700 text-white"
            >
              <ng-icon name="lucideSave" size="14" class="mr-1" />
              {{ savingProcedure() ? 'Submitting...' : 'Submit Procedure Order' }}
            </button>
          </div>
        </div>
      </div>
      ...
      >
        <div
          class="w-full max-w-xl rounded-xl border border-border bg-card p-6 shadow-lg space-y-4"
        >
          <div class="flex justify-between items-center border-b border-border pb-3">
            <h3 class="text-sm font-bold text-foreground flex items-center gap-2">
          ...
          <div class="space-y-3 text-xs">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. Inpatient Day 2 Progress Note"
                class="w-full p-2 rounded-md border border-input bg-background"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Document subjective findings, objective labs/vitals, clinical assessment, and treatment plan..."
                class="w-full p-2 rounded-md border border-input bg-background h-36"
              ></textarea>
            </div>
          </div>

          <div class="flex justify-end gap-2 pt-2 border-t border-border">
            <button hlmBtn variant="outline" size="sm" (click)="showDocModal.set(false)">
              Cancel
            </button>
            <button
              hlmBtn
              variant="default"
              size="sm"
              [disabled]="savingDoc() || !newDoc.title || !newDoc.content"
              (click)="saveClinicalDoc()"
              class="bg-emerald-600 hover:bg-emerald-700 text-white"
            >
              <ng-icon name="lucideSave" size="14" class="mr-1" />
        ...
        >
          <div class="flex justify-between items-center border-b border-border pb-3">
            ...
            <button
              ...
              variant="ghost"
              size="sm"
              (click)="showCareTeamModal.set(false)"
              class="size-7 p-0"
            >
              <ng-icon name="lucideX" size="16" />
            </button>
          ...
          <div class="space-y-3 text-xs">
            <div>
              <label class="font-medium text-foreground block mb-1"
                >Clinician / Practitioner Email *</label
              >
              <input
                type="text"
                [(ngModel)]="newCareMember.email"
                placeholder="e.g. dr_smith, nurse_sarah"
                class="w-full p-2 rounded-md border border-input bg-background"
              />
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                [(ngModel)]="newCareMember.fullName"
            ...
            <div>
              ...
              <input
                type="text"
                [(ngModel)]="newCareMember.specialty"
                placeholder="e.g. Cardiology, Critical Care, Nephrology"
                class="w-full p-2 rounded-md border border-input bg-background"
              />
            </div>
          </div>
          ...
          <div class="flex justify-end gap-2 pt-2 border-t border-border">
            <button hlmBtn variant="outline" size="sm" (click)="showCareTeamModal.set(false)">
              Cancel
            </button>
            <button
              hlmBtn
              variant="default"
              size="sm"
              [disabled]="savingCareMember() || !newCareMember.email"
              (click)="saveCareMember()"
              class="bg-cyan-600 hover:bg-cyan-700 text-white"
            >
              <ng-icon name="lucideSave" size="14" class="mr-1" />
              {{ savingCareMember() ? 'Assigning...' : 'Assign Member' }}
            </button>
          </div>
        </div>
      </div>
        ...
        >
          <div class="flex justify-between items-center border-b border-border pb-3">
            ...
            <button
              ...
              variant="ghost"
              size="sm"
              (click)="showConsentModal.set(false)"
              class="size-7 p-0"
            >
              <ng-icon name="lucideX" size="16" />
            </button>
          ...
          <div class="space-y-3 text-xs">
            <div>
              ...
              <select
                [(ngModel)]="newConsent.consentTypeId"
                class="w-full p-2 rounded-md border border-input bg-background"
              >
            ...
            <div>
              ...
              <input
                type="text"
                [(ngModel)]="newConsent.signedByPatientName"
            ...
            <div>
              ...
              <input
                type="text"
                [(ngModel)]="newConsent.witnessName"
                placeholder="Attending clinician or witness"
                class="w-full p-2 rounded-md border border-input bg-background"
              />
            </div>
          </div>
          ...
          <div class="flex justify-end gap-2 pt-2 border-t border-border">
            <button hlmBtn variant="outline" size="sm" (click)="showConsentModal.set(false)">
              Cancel
            </button>
            <button
              hlmBtn
              variant="default"
              size="sm"
              [disabled]="savingConsent() || !newConsent.signedByPatientName"
              (click)="savePatientConsent()"
              class="bg-purple-600 hover:bg-purple-700 text-white"
            >
              <ng-icon name="lucideSave" size="14" class="mr-1" />
              {{ savingConsent() ? 'Signing...' : 'Execute Consent' }}
            </button>
          </div>
        </div>
      </div>
...
export class PhysicianChartComponent implements OnInit {
  ...
  saveDiagnosis(): void {
    ...
    if (
      !active?.id ||
      !this.newDiagnosis.icdCode ||
      !this.newDiagnosis.conditionName ||
      this.savingDiagnosis()
    )
      return;
  ...
  saveErx(): void {
    const active = this.patientContext.activePatient();
    if (!active?.id || !this.newErx.medicationName || !this.newErx.dosage || this.savingErx())
      return;
  ...
  saveVitals(): void {
    ...
    this.apiService
      .recordVitals({
        ...
        oxygenSaturation: this.newVitals.oxygenSaturation
          ? Number(this.newVitals.oxygenSaturation)
          : undefined,
  ...
  finalizeDoc(docId: number | string): void {
    this.apiService.finalizeClinicalDocument(docId).subscribe({
      next: (doc) => {
        ...
        this.clinicalDocuments.update((list) =>
          list.map((d) => (d.id === docId ? { ...d, status: 'FINAL' } : d)),
        );
      },
    });
  }
  ...
  revokeConsent(consentId: number | string): void {
    this.apiService
      .revokePatientConsent(consentId, 'Revoked by clinician upon patient request')
      .subscribe({
        next: () => {
          toast.success('Consent revoked');
          this.patientConsents.update((list) =>
            list.map((c) => (c.id === consentId ? { ...c, status: 'REVOKED' } : c)),
          );
        },
      });
  }
