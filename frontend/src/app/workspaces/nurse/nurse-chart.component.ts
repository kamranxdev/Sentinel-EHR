...
export type NurseChartTab =
  'vitals' | 'mar' | 'io' | 'assessment' | 'allergies' | 'sbar' | 'care-team';
    ...
    <div class="w-full space-y-6">
      ...
      <div class="rounded-2xl border border-border bg-card p-5 shadow-xs space-y-4">
          ...
          <div>
            <h1 class="text-xl font-bold tracking-tight text-foreground flex items-center gap-2">
              ...
              <span
                hlmBadge
                variant="secondary"
                class="text-[10px] bg-primary/10 text-primary border-primary/20"
              >
                Bedside Context
              </span>
            </h1>
            <p class="text-xs text-muted-foreground mt-0.5">
              Comprehensive nursing chart for physiological vitals, eMAR administrations, I/O fluid
              balance, head-to-toe assessments, and SBAR handoffs.
            </p>
          </div>

          <!-- Patient Selector -->
          <div
            class="flex items-center gap-2 bg-muted/40 border border-border rounded-xl p-1.5 w-full md:w-auto"
          >
        ...
        <div *ngIf="activePatient() as patient; else noPatientSelected" class="space-y-3">
          <div
            class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3 text-xs bg-muted/20 p-3.5 rounded-xl border border-border/60"
          >
            <div>
              <span
            ...
            <div>
              ...
              <span
                hlmBadge
                variant="outline"
                class="text-[10px] font-semibold text-emerald-600 dark:text-emerald-400 border-emerald-500/30 bg-emerald-500/10"
              >
                {{ patient.bloodType || 'A+' }}
              </span>
            </div>
          ...
          <!-- Safety Alerts Strip -->
          <div
            class="flex items-center flex-wrap gap-2 px-3 py-2 bg-muted/40 rounded-xl border border-border text-xs"
          >
            <span
            ...
            <span
              hlmBadge
              variant="outline"
              class="text-[10px] font-bold text-rose-600 border-rose-500/30 bg-rose-500/10"
            >
              ● Penicillin Allergy
            </span>
            ...
            <span
              hlmBadge
              variant="outline"
              class="text-[10px] font-bold text-emerald-600 border-emerald-500/30 bg-emerald-500/10"
            >
              Code: FULL CODE
            </span>
            ...
            <span
              hlmBadge
              variant="outline"
              class="text-[10px] font-bold text-amber-600 border-amber-500/30 bg-amber-500/10"
            >
              Fall Risk: Moderate
            </span>
            ...
            <span
              hlmBadge
              variant="outline"
              class="text-[10px] font-bold text-blue-600 border-blue-500/30 bg-blue-500/10"
            >
        ...
        <ng-template #noPatientSelected>
          <div
            class="p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl text-amber-700 dark:text-amber-300 text-xs flex items-center gap-2"
          >
            <ng-icon name="lucideTriangleAlert" size="16" />
            <span
              >No active patient selected. Please choose a patient from the dropdown above.</span
            >
          </div>
        </ng-template>
        ...
        <div
          class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-1.5 p-1.5 bg-muted/40 rounded-xl border border-border"
        >
          <button
      ...
      <div *ngIf="activeTab() === 'vitals'" class="space-y-6">
        ...
        >
          <div>
            ...
            <p class="text-xs text-muted-foreground mt-0.5">
              Real-time vital parameters recorded at bedside with NEWS2 early warning trend scoring.
            </p>
          </div>
          ...
          <button
            ...
            class="gap-1.5 font-semibold text-xs bg-primary hover:bg-primary/90 text-primary-foreground shadow-xs"
          >
            ...
            <table hlmTable class="w-full text-xs">
              ...
              <tbody hlmTableBody class="divide-y divide-border">
                ...
                >
                  ...
                  <td hlmTableCell class="py-3 px-4">
                    <span
                      hlmBadge
                      variant="secondary"
                      class="text-[10px] font-bold bg-emerald-500/10 text-emerald-600"
                    >
                      Score: 1 (STABLE)
                    </span>
                  </td>
                ...
                <tr *ngIf="vitals().length === 0" hlmTableRow>
                  ...
                  >
                    No vitals logged for this patient. Click Log Bedside Vitals above.
                  </td>
                </tr>
              </tbody>
            </table>
      ...
      <div *ngIf="activeTab() === 'mar'" class="space-y-6">
        ...
        >
          <div>
            ...
            <p class="text-xs text-muted-foreground mt-0.5">
              Verify 5-Rights of medication administration and record dose execution or refusal.
            </p>
          </div>
        </div>
      ...
      <div *ngIf="activeTab() === 'io'" class="space-y-6">
        ...
        >
          <div>
            ...
            <p class="text-xs text-muted-foreground mt-0.5">
              Track enteral/IV fluid intake against urinary, emesis, and surgical drain output.
            </p>
          </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div class="p-4 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 space-y-1">
            <span class="text-[10px] font-bold uppercase tracking-wider text-emerald-600"
              >Net Fluid Balance</span
            >
            <div class="text-2xl font-extrabold text-emerald-600">
              {{ netFluidBalance() > 0 ? '+' : '' }}{{ netFluidBalance() }} mL
            </div>
            ...
            <table hlmTable class="w-full text-xs">
              ...
              <tbody hlmTableBody class="divide-y divide-border">
                ...
                >
                  ...
                  <td hlmTableCell class="py-3 px-4 font-mono text-amber-600">
                    {{ io.urineMl }} mL
                  </td>
                  <td hlmTableCell class="py-3 px-4 font-mono text-amber-600">
                    {{ io.drainMl }} mL
      ...
      <div *ngIf="activeTab() === 'assessment'" class="space-y-6">
        ...
        >
          <div>
            ...
            <p class="text-xs text-muted-foreground mt-0.5">
              Document comprehensive clinical systems assessments for shift continuity.
            </p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="p-4 rounded-2xl border border-border bg-card space-y-2">
            ...
            <p class="text-xs text-muted-foreground">
              Alert and oriented x4. Pupils equal, round, and reactive to light (PERRLA). GCS:
              15/15.
            </p>
          </div>
          ...
          <div class="p-4 rounded-2xl border border-border bg-card space-y-2">
            ...
            <p class="text-xs text-muted-foreground">
              Lungs clear to auscultation bilaterally. Room air SpO2 98%. No retractions or
              wheezing.
            </p>
          </div>
          ...
          <div class="p-4 rounded-2xl border border-border bg-card space-y-2">
            <h3 class="text-xs font-bold uppercase tracking-wider text-foreground">
              3. Cardiovascular & Perfusion
            </h3>
            <p class="text-xs text-muted-foreground">
              Regular rate and rhythm. S1/S2 present. Capillary refill < 2s. Peripheral pulses +2
              bilaterally.
            </p>
          </div>
          ...
          <div class="p-4 rounded-2xl border border-border bg-card space-y-2">
            <h3 class="text-xs font-bold uppercase tracking-wider text-foreground">
              4. Skin, Wounds & Fall Risk
            </h3>
            <p class="text-xs text-muted-foreground">
              Skin warm, dry, and intact. Surgical incision clean and dry, dressed with gauze.
              Braden Score: 18.
            </p>
          </div>
        </div>
      ...
      <div *ngIf="activeTab() === 'allergies'" class="space-y-6">
        ...
        >
          <div>
            ...
            <p class="text-xs text-muted-foreground mt-0.5">
              Verified allergen registry for clinical safety and cross-reactivity prevention.
            </p>
          </div>
        </div>
        ...
        <div class="p-4 rounded-2xl border border-rose-500/30 bg-rose-500/5 space-y-3">
          ...
          <div class="text-xs space-y-1">
            ...
            <div class="text-muted-foreground">
              Reaction: Anaphylaxis / Severe Hives • Severity: HIGH • Verified by Nursing Intake
            </div>
          </div>
        </div>
      </div>
      ...
      <div *ngIf="activeTab() === 'sbar'" class="space-y-6">
        ...
        >
          <div>
            ...
            <p class="text-xs text-muted-foreground mt-0.5">
              Standardized shift-to-shift handoff (Situation, Background, Assessment,
              Recommendation).
            </p>
          </div>
          ...
          <button
            ...
            variant="default"
            size="sm"
            (click)="signSbarHandoff()"
            class="gap-1.5 font-semibold text-xs bg-emerald-600 hover:bg-emerald-700 text-white shadow-xs"
          >
        ...
        <div class="p-5 rounded-2xl border border-border bg-card space-y-4 text-xs">
          <div class="space-y-1">
            <h4 class="font-bold text-primary uppercase tracking-wider text-[11px]">
              S - Situation
            </h4>
            <p class="text-muted-foreground">
              62-year-old patient admitted with Acute Coronary Syndrome on Day 2 of observation
              under Dr. Sharma.
            </p>
          </div>
          ...
          <div class="space-y-1 pt-3 border-t border-border">
            ...
            <p class="text-muted-foreground">
              History of hypertension and dyslipidemia. Allergic to Penicillin. Started on Dual
              Antiplatelet Therapy and IV fluids.
            </p>
          </div>
          ...
          <div class="space-y-1 pt-3 border-t border-border">
            ...
            <p class="text-muted-foreground">
              Vitals stable (BP 124/78, HR 68, SpO2 99%). NEWS2 Score: 1. Net fluid balance +400 mL
              over shift. Pain 0/10.
            </p>
          </div>
          ...
          <div class="space-y-1 pt-3 border-t border-border">
            ...
            <p class="text-muted-foreground">
              Evening dose of Metformin due at 18:00. Fasting lipid panel scheduled for 06:00
              tomorrow morning.
            </p>
          </div>
        </div>
      ...
      <div *ngIf="activeTab() === 'care-team'" class="space-y-6">
        ...
        >
          <div>
            ...
            <p class="text-xs text-muted-foreground mt-0.5">
              Assigned clinicians, attending doctors, and bedside nursing coverage.
            </p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <div class="p-4 rounded-2xl border border-border bg-card space-y-2">
            ...
            <span hlmBadge variant="secondary" class="text-[10px] bg-primary/10 text-primary"
              >Attending Cardiologist</span
            >
            <div class="text-[11px] text-muted-foreground">
              Extension: #4402 • Main Medical Center
            </div>
          </div>
          ...
          <div class="p-4 rounded-2xl border border-border bg-card space-y-2">
            ...
            <span
              ...
              >Primary Bedside Nurse</span
            >
            <div class="text-[11px] text-muted-foreground">
              Ward 3A • Morning Shift (07:00–15:00)
            </div>
          </div>

          <div class="p-4 rounded-2xl border border-border bg-card space-y-2">
            <div class="font-bold text-foreground text-xs">Pharm. D. Miller</div>
            <span hlmBadge variant="secondary" class="text-[10px] bg-purple-500/10 text-purple-600"
              >Clinical Pharmacist</span
            >
