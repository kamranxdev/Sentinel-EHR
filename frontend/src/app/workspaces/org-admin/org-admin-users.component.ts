      ...
      <div class="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 pb-4 border-b border-border">
        <div>
          <h1 class="text-xl font-bold tracking-tight text-foreground flex items-center gap-2">
            ...
            <span hlmBadge variant="secondary" class="text-[10px]">ROLE_ORG_ADMIN</span>
          </h1>
...
export class OrgAdminUsersComponent implements OnInit {
  ...
  staffCount = computed(() => this.users().filter((u) => {
    ...
    return roles.includes('ROLE_ADMIN') || roles.includes('ROLE_ORG_ADMIN') || roles.includes('ROLE_RECEPTIONIST');
  }).length);
  ...
  filteredUsers = computed(() => {
    ...
    if (tab === 'DOCTOR') list = list.filter((u) => this.getUserRoleNames(u).includes('ROLE_DOCTOR'));
      ...
      return r.includes('ROLE_ADMIN') || r.includes('ROLE_ORG_ADMIN') || r.includes('ROLE_RECEPTIONIST');
    });
  ...
  getUserAvatarBg(u: User): string {
    ...
    if (roles.includes('ROLE_ORG_ADMIN')) return 'bg-purple-500/20 text-purple-600';
