        ...
        <div>
          <div class="flex items-center gap-2">
            ...
            <span class="text-xs text-muted-foreground font-mono"
              >Patient Census & Registration Policies</span
            >
          </div>
          ...
          <p class="text-xs text-muted-foreground mt-0.5">
            Administrative registry oversight, insurance coverage ratios, ABHA linkages, and patient
            registration metrics for the organization.
          </p>
        </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="p-4 rounded-xl border border-border bg-card shadow-xs">
          ...
          <div class="mt-2 flex items-baseline justify-between">
            <span class="text-2xl font-bold text-emerald-600 font-mono">{{
              patients().length
            }}</span>
        ...
        <div class="p-4 rounded-xl border border-border bg-card shadow-xs">
          <div class="flex items-center justify-between text-muted-foreground">
            ...
            <div
              class="size-9 rounded-lg bg-purple-500/10 text-purple-600 flex items-center justify-center"
            >
        ...
        >
          <div>
            <h2 class="text-sm font-bold text-foreground">
              Organization Administrative Registry Census
            </h2>
            <p class="text-xs text-muted-foreground">
              Administrative MRN roster with demographic and insurance policy markers (Zero Clinical
              Data Exposure).
            </p>
          </div>
            ...
            <tbody class="divide-y divide-border">
              <tr *ngFor="let p of filteredPatients()" class="hover:bg-muted/30 transition-colors">
                <td class="py-3.5 px-4 font-semibold text-foreground">
                  <div class="flex items-center gap-2">
                    <span
                      class="size-7 rounded-full bg-emerald-500/10 text-emerald-600 font-bold text-xs flex items-center justify-center"
                    >
                      {{ (p.fullName || 'P')[0] }}
                    </span>
                    <div>
                ...
                <td class="py-3.5 px-4 font-mono text-muted-foreground">
                  <div *ngIf="p.abhaId" class="text-sky-600 font-semibold text-[11px]">
                    {{ p.abhaId }}
                  </div>
                ...
                <td class="py-3.5 px-4">
                  <span
                    [ngClass]="
                      p.insuranceProvider && p.insuranceProvider !== 'Self-Pay'
                ...
                <td class="py-3.5 px-4 text-right">
                  <span
                    class="px-2 py-0.5 rounded text-[10px] font-mono bg-muted text-muted-foreground border border-border"
                  >
                    Admin Census Only
                  </span>
                </td>
...
export class OrganizationAdminPatientsComponent implements OnInit {
  ...
  insuredCount = computed(
    () =>
      this.patients().filter((p) => p.insuranceProvider && p.insuranceProvider !== 'Self-Pay')
        .length,
  );
  insuredPct = computed(() => {
    const total = this.patients().length;
