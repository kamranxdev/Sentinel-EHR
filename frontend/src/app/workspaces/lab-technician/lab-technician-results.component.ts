  ...
  template: `
    <div class="space-y-6">
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-xs text-muted-foreground mt-0.5">
              Quantitative analyte parameter validation, LOINC mapping, delta check engine, and mandatory critical panic telephone escalation.
            </p>
          </div>
        </div>

        <div class="flex items-center gap-2">
          <a routerLink="/lab-technician/worklist" hlmBtn variant="outline" size="sm" class="text-xs">
            Worklist Board
          </a>
        </div>
      </div>
        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
          <div class="space-y-1.5 flex-1">
            <div class="flex items-center gap-2 flex-wrap">
              <span class="font-mono text-sm font-extrabold text-foreground">#LAB-{{ selectedOrderId }}</span>
              <span
                hlmBadge
              ...
              <span *ngIf="selectedOrder?.specimenBarcode" class="font-mono text-[11px] bg-muted px-2 py-0.5 rounded border border-border flex items-center gap-1">
          ...
          <div *ngIf="availableOrders().length > 0" class="flex items-center gap-2">
            ...
            >
              <option *ngFor="let ord of availableOrders()" [value]="ord.id">
                #LAB-{{ ord.id }} - {{ ord.patientFullName || ord.patient?.fullName || 'Patient' }} ({{ ord.testName }})
              </option>
            </select>
          </div>
        </div>
      ...
      <form (ngSubmit)="submitResults()" class="space-y-6">
        ...
        <div hlmCard class="p-5 space-y-4">
          <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-3 border-b border-border">
            <div>
              ...
              <p class="text-xs text-muted-foreground">Select a standard LOINC test panel preset or configure customized quantitative analytes.</p>
            </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
            <div class="space-y-1">
              ...
              <input hlmInput type="text" [(ngModel)]="testName" name="testName" required class="w-full text-xs" />
            </div>

            <div class="space-y-1">
              <label class="font-semibold text-foreground">Primary Battery LOINC Code *</label>
              <input hlmInput type="text" [(ngModel)]="primaryLoinc" name="primaryLoinc" required class="w-full text-xs font-mono" />
            </div>

            <div class="space-y-1">
              <label class="font-semibold text-foreground">Laboratory Discipline</label>
              <select [(ngModel)]="testCategory" name="testCategory" class="w-full h-8 rounded-md border border-input bg-background px-2 text-xs">
        ...
        <div hlmCard class="p-5 space-y-4">
          <div class="flex items-center justify-between pb-3 border-b border-border">
            <div>
              ...
              <p class="text-xs text-muted-foreground">Values are evaluated against reference ranges and compared to baseline records in real-time.</p>
            </div>
          ...
          <div class="overflow-x-auto rounded-lg border border-border">
            <table hlmTable class="w-full text-xs">
              ...
              <tbody hlmTableBody class="divide-y divide-border">
                <tr *ngFor="let comp of components; let i = index" hlmTableRow class="hover:bg-muted/30 transition-colors">
                  <!-- Analyte Name -->
                  ...
                  <td hlmTableCell class="py-2 px-3">
                    <input
                      ...
                      [class.text-amber-600]="comp.abnormalFlag === 'HIGH' || comp.abnormalFlag === 'LOW'"
                    />
                  </td>
                  ...
                  <!-- Delta Shift Check -->
                  <td hlmTableCell class="py-2 px-3">
                    <div *ngIf="comp.previousValue !== undefined && comp.previousValue !== null" class="space-y-0.5">
                      ...
                      <div
                        *ngIf="comp.deltaPercent !== null && comp.deltaPercent !== undefined"
                        ...
                        [class.text-amber-600]="Math.abs(comp.deltaPercent) > 15 && Math.abs(comp.deltaPercent) <= 30"
                        [class.text-emerald-600]="Math.abs(comp.deltaPercent) <= 15"
                      >
                        {{ comp.deltaPercent > 0 ? '+' : '' }}{{ comp.deltaPercent | number: '1.1-1' }}%
                        <span *ngIf="Math.abs(comp.deltaPercent) > 30" class="text-[9px] uppercase tracking-wide bg-rose-500/10 px-1 rounded">Delta Alert</span>
                      </div>
                    </div>
                    <span *ngIf="comp.previousValue === undefined || comp.previousValue === null" class="text-muted-foreground/50 text-[11px]">
                      First Test Baseline
                    </span>
                  </td>
        ...
        <!-- ========================================================================= -->
        <div *ngIf="hasPanicValues()" class="p-5 rounded-xl border border-rose-500/40 bg-rose-500/5 space-y-4 shadow-sm">
          <div class="flex items-center gap-3">
            <div class="size-10 rounded-lg bg-rose-500/20 text-rose-600 dark:text-rose-400 flex items-center justify-center shrink-0 animate-bounce">
              <ng-icon name="lucidePhoneCall" size="22" />
            </div>
            <div>
              <h3 class="text-sm font-bold text-rose-600 dark:text-rose-400 flex items-center gap-2">
                <span>MANDATORY CLINICAL TELEPHONE ESCALATION REQUIRED</span>
                <span hlmBadge variant="destructive" class="text-[9px] animate-pulse">PANIC VALUE</span>
              </h3>
              <p class="text-xs text-muted-foreground">
                One or more analytes have exceeded panic limits. Protocol mandates immediate telephone contact with the ordering clinician and verbal read-back.
              </p>
            </div>
          </div>

          <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs pt-2 border-t border-rose-500/20">
            <div class="space-y-1">
            ...
            <div class="space-y-1 sm:col-span-3">
              <label class="flex items-center gap-2 text-xs font-bold text-foreground cursor-pointer select-none">
                ...
                />
                <span>Physician Verbal Read-Back Confirmed (Clinician verbally verified critical analyte values) *</span>
              </label>
            </div>
          </div>
        </div>
        ...
        <div hlmCard class="p-5 space-y-4">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            <div class="space-y-1 sm:col-span-2">
              <label class="font-semibold text-foreground">Clinical Interpretation & Pathologist Remarks</label>
              <textarea
            ...
            <div class="space-y-1">
              ...
              <select [(ngModel)]="verifierRole" name="verifierRole" class="w-full h-8 rounded-md border border-input bg-background px-2 text-xs">
                <option value="ROLE_PATHOLOGIST">Senior Pathologist (MD / FRCPath)</option>
                <option value="ROLE_LAB_TECH">Certified Medical Laboratory Technologist (MLT)</option>
              </select>
            </div>
          ...
          <div class="pt-4 flex justify-end gap-3 border-t border-border">
            <a routerLink="/lab-technician/worklist" hlmBtn variant="outline" size="sm" class="text-xs">Cancel</a>
            <button
              hlmBtn
              variant="default"
              type="submit"
              [disabled]="submitting() || !selectedOrderId || (hasPanicValues() && !readBackConfirmed)"
              class="gap-2 text-xs bg-teal-600 hover:bg-teal-700 text-white"
            >
              <ng-icon name="lucideCheckCheck" size="15" />
              <span>{{ submitting() ? 'Publishing Results to Chart...' : 'Verify & Release to Doctor Chart' }}</span>
            </button>
          </div>
        </div>
      </form>
...
export class LabTechnicianResultsComponent implements OnInit {
  ...
  pathologistNotes = 'Electrolytes and renal profile verified. Findings transmitted to patient chart.';
  ...
  presets: TestPanelPreset[] = [
    {
      ...
      category: 'CHEMISTRY',
      components: [
        { code: '2345-7', name: 'Glucose', defaultVal: 104, unit: 'mg/dL', refLow: 70, refHigh: 99, panicLow: 40, panicHigh: 450, prevVal: 98 },
        { code: '3094-0', name: 'Blood Urea Nitrogen (BUN)', defaultVal: 18, unit: 'mg/dL', refLow: 7, refHigh: 20, panicLow: 2, panicHigh: 100, prevVal: 16 },
        { code: '2160-0', name: 'Creatinine', defaultVal: 1.1, unit: 'mg/dL', refLow: 0.7, refHigh: 1.3, panicLow: 0.3, panicHigh: 5.0, prevVal: 1.0 },
        { code: '2951-2', name: 'Sodium (Na+)', defaultVal: 140, unit: 'mmol/L', refLow: 136, refHigh: 145, panicLow: 120, panicHigh: 160, prevVal: 139 },
        { code: '2823-3', name: 'Potassium (K+)', defaultVal: 4.3, unit: 'mmol/L', refLow: 3.5, refHigh: 5.1, panicLow: 2.8, panicHigh: 6.5, prevVal: 4.1 },
        { code: '2075-0', name: 'Chloride (Cl-)', defaultVal: 101, unit: 'mmol/L', refLow: 98, refHigh: 107, panicLow: 80, panicHigh: 120, prevVal: 102 },
        { code: '2028-9', name: 'Carbon Dioxide (CO2)', defaultVal: 24, unit: 'mmol/L', refLow: 22, refHigh: 29, panicLow: 10, panicHigh: 40, prevVal: 25 },
        { code: '17861-6', name: 'Calcium', defaultVal: 9.4, unit: 'mg/dL', refLow: 8.6, refHigh: 10.3, panicLow: 6.5, panicHigh: 13.0, prevVal: 9.2 },
      ],
    },
    {
      ...
      category: 'HEMATOLOGY',
      components: [
        { code: '6690-2', name: 'White Blood Cells (WBC)', defaultVal: 7.2, unit: 'x10^3/uL', refLow: 4.5, refHigh: 11.0, panicLow: 2.0, panicHigh: 30.0, prevVal: 6.8 },
        { code: '789-8', name: 'Red Blood Cells (RBC)', defaultVal: 4.8, unit: 'x10^6/uL', refLow: 4.3, refHigh: 5.9, panicLow: 2.0, panicHigh: 7.0, prevVal: 4.7 },
        { code: '718-7', name: 'Hemoglobin (HGB)', defaultVal: 14.8, unit: 'g/dL', refLow: 13.5, refHigh: 17.5, panicLow: 7.0, panicHigh: 20.0, prevVal: 15.0 },
        { code: '4544-3', name: 'Hematocrit (HCT)', defaultVal: 43.5, unit: '%', refLow: 38.8, refHigh: 50.0, panicLow: 20.0, panicHigh: 60.0, prevVal: 44.0 },
        { code: '777-3', name: 'Platelets (PLT)', defaultVal: 245, unit: 'x10^3/uL', refLow: 150, refHigh: 450, panicLow: 20, panicHigh: 1000, prevVal: 230 },
      ],
    },
    {
      name: 'Hemoglobin A1c',
      ...
      components: [
        { code: '4548-4', name: 'Hemoglobin A1c (HbA1c)', defaultVal: 6.8, unit: '%', refLow: 4.0, refHigh: 5.6, panicLow: 3.5, panicHigh: 14.0, prevVal: 7.4 },
        { code: '27161-9', name: 'Estimated Avg Glucose (eAG)', defaultVal: 149, unit: 'mg/dL', refLow: 70, refHigh: 114, prevVal: 166 },
      ],
    },
    {
      ...
      category: 'ENDOCRINOLOGY',
      components: [
        { code: '89579-7', name: 'High-Sensitivity Troponin I', defaultVal: 0.02, unit: 'ng/mL', refLow: 0.0, refHigh: 0.04, panicHigh: 0.10, prevVal: 0.01 },
      ],
    },
  ];
      ...
      next: (orders) => {
        ...
        this.route.queryParams.subscribe((params) => {
          if (params['orderId']) {
            ...
            this.patientName = this.selectedOrder.patientFullName || this.selectedOrder.patient?.fullName || 'Patient';
          }
        });
      },
      error: () => {},
  ...
  matchPresetByTestName(name: string): void {
    ...
    } else if (lower.includes('bmp') || lower.includes('metabolic') || lower.includes('electrolyte')) {
  ...
  applyPreset(preset: TestPanelPreset): void {
    ...
    this.components = preset.components.map((c) => {
      const comp: LabResultComponent = {
        code: c.code,
        name: c.name,
        valueNumeric: typeof c.defaultVal === 'number' ? c.defaultVal : parseFloat(String(c.defaultVal)) || null,
  ...
  submitResults(): void {
    ...
    if (this.hasPanicValues() && !this.readBackConfirmed) {
      toast.error('Protocol violation: Verbal read-back confirmation with clinician is mandatory for critical panic values.');
      return;
    }
    ...
    const hasAbnormal = this.components.some((c) => c.abnormalFlag === 'HIGH' || c.abnormalFlag === 'LOW' || c.abnormalFlag === 'CRITICAL_PANIC');
    ...
    const payload = {
      ...
      unit: primaryUnit,
      referenceRange: this.components.length > 0 ? `${this.components[0].referenceLow} - ${this.components[0].referenceHigh}` : '',
    ...
    this.apiService.addLabResult(this.selectedOrderId, payload).subscribe({
      next: (res) => {
        ...
        this.apiService.updateLabOrderStatus(this.selectedOrderId, 'VERIFIED').subscribe({ error: () => {} });

        this.submitting.set(false);
        toast.success(`Lab result for Order #LAB-${this.selectedOrderId} verified and published to Doctor Chart.`);
        this.router.navigate(['/lab-technician/worklist'], { queryParams: { stage: 'RESULTED' } });
      },
      error: () => {
        // Fallback update order status
        this.apiService.updateLabOrderStatus(this.selectedOrderId, 'RESULTED').subscribe({ error: () => {} });
        this.submitting.set(false);
