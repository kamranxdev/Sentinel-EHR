...
import { MedicationOrder, MedicationBatch, DispensationRecord } from '../../core/models/pharmacy.model';
      ...
      <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 pb-4 border-b border-border">
        <div>
          <div class="flex items-center gap-2">
            ...
            <span class="text-xs text-muted-foreground font-mono">Medication Dispensing & Stock Allocation</span>
          </div>
          <h1 class="text-2xl font-bold tracking-tight text-foreground mt-1">
            Medication Dispensation Station
          </h1>
          <p class="text-xs text-muted-foreground mt-0.5">
            Select physical medication batch (FEFO), deduct pharmaceutical inventory, and dispatch billable medication charges.
          </p>
        </div>
      ...
      <div class="bg-card rounded-2xl border border-border shadow-xs overflow-hidden">
        <div class="p-4 border-b border-border bg-muted/20 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <div>
            ...
            <p class="text-xs text-muted-foreground">Orders verified by clinical pharmacist awaiting physical batch fulfillment.</p>
          </div>
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-xs text-left">
            ...
            <tbody class="divide-y divide-border">
              <tr *ngFor="let o of filteredVerifiedOrders()" class="hover:bg-muted/30 transition-colors">
                <td class="py-3.5 px-4 font-semibold text-foreground">
                  <div class="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div class="text-[11px] font-normal text-muted-foreground">{{ o.dosage }}</div>
                    </div>
                  </div>
                </td>
                <td class="py-3.5 px-4">
                  <div class="font-medium text-foreground">{{ o.patient?.fullName || 'Patient' }}</div>
                ...
                <td class="py-3.5 px-4">
                  <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-500/15 text-emerald-600 border border-emerald-500/30">
                    VERIFIED & APPROVED
                  </span>
                </td>
        ...
        <div class="w-full max-w-lg rounded-2xl border border-border bg-card p-6 shadow-2xl space-y-4">
          ...
          <div class="space-y-3.5 text-xs">
            <div class="p-3 bg-muted/40 rounded-xl space-y-1 text-[11px] font-mono">
              <div><strong>Patient:</strong> {{ selectedOrder()?.patient?.fullName }} (MRN: {{ selectedOrder()?.patient?.patientCode }})</div>
              <div><strong>Dosage & Frequency:</strong> {{ selectedOrder()?.dosage }} &bull; {{ selectedOrder()?.frequency }}</div>
              <div><strong>Prescribed Quantity:</strong> {{ selectedOrder()?.quantity }} units</div>
            </div>

            <!-- Batch Selection -->
            <div>
              <label class="block font-semibold text-foreground mb-1">Select Physical Stock Batch (FEFO First-Expiring) *</label>
              <select [(ngModel)]="dispenseForm.batchId" class="w-full px-3 py-2 rounded-lg border border-input bg-background font-mono text-xs">
                <option *ngFor="let b of batches()" [value]="b.id">
                  Batch #{{ b.batchNumber }} - Exp: {{ b.expiryDate }} (Avail: {{ b.quantityOnHand }} units) [{{ b.location }}]
                </option>
              </select>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="number" [(ngModel)]="dispenseForm.quantity" class="w-full px-3 py-2 rounded-lg border border-input bg-background font-mono" />
              </div>
              <div>
                <label class="block font-semibold text-foreground mb-1">Automated Charge Trigger</label>
                <div class="px-3 py-2 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 font-semibold font-mono text-[11px] flex items-center gap-1">
            ...
            <div>
              <label class="block font-semibold text-foreground mb-1">Dispensation Instructions / Note</label>
              <input type="text" [(ngModel)]="dispenseForm.notes" placeholder="e.g. Take with meals. Patient instructed on adverse symptoms." class="w-full px-3 py-2 rounded-lg border border-input bg-background" />
            </div>
          </div>

          <div class="flex justify-end gap-2 pt-3 border-t border-border">
            <button (click)="selectedOrder.set(null)" class="px-4 py-2 rounded-lg border border-border text-xs font-semibold">Cancel</button>
            <button (click)="submitDispense()" class="px-4 py-2 rounded-lg bg-emerald-600 text-white text-xs font-semibold hover:bg-emerald-700 transition-colors shadow-xs">
...
export class PharmacistDispenseComponent implements OnInit {
  ...
  filteredVerifiedOrders = computed(() => {
    let list = this.orders().filter((o) => o.status === 'PHARMACY_VERIFIED' || o.status === 'VERIFIED');
  ...
  submitDispense(): void {
    ...
    this.apiService.dispensePharmacyOrder(o.id, this.dispenseForm).subscribe({
      next: () => {
        toast.success(`Medication ${o.medicationName} dispensed successfully. Stock deducted & billable charge created.`);
        o.status = 'DISPENSED';
