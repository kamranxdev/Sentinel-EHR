...
import { InventoryItem, MedicationBatch, StockReceiptDTO, StockAdjustmentDTO } from '../../core/models/pharmacy.model';
...
@Component({
  ...
  template: `
    <div class="space-y-6">
      ...
      <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 pb-4 border-b border-border">
        <div>
          <div class="flex items-center gap-2">
            ...
            <span class="text-xs text-muted-foreground font-mono">Pharmaceutical Inventory & FEFO Batches</span>
          </div>
          <h1 class="text-2xl font-bold tracking-tight text-foreground mt-1">
            Medication Inventory & Batch Management
          </h1>
          <p class="text-xs text-muted-foreground mt-0.5">
            Monitor inventory on hand, track physical batches and expiration dates, record shipments, and manage stock buffer thresholds.
          </p>
        </div>
      ...
      <div class="flex flex-col sm:flex-row justify-between gap-4">
        ...
        <div class="flex items-center gap-2">
          <button
            ...
            [ngClass]="selectedFilter() === 'ALL' ? 'bg-purple-600 text-white' : 'bg-card text-muted-foreground hover:text-foreground'"
          ...
          <button
            ...
            [ngClass]="selectedFilter() === 'LOW_STOCK' ? 'bg-destructive text-destructive-foreground' : 'bg-card text-destructive hover:bg-destructive/10'"
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left text-xs">
            ...
            <tbody class="divide-y divide-border">
              <tr *ngFor="let item of filteredInventory()" class="hover:bg-muted/30 transition-colors">
                <td class="py-3.5 px-4 font-semibold text-foreground">
                  <div class="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div class="text-[11px] font-normal text-muted-foreground">{{ item.genericName }} &bull; {{ item.strength }}</div>
                    </div>
                  </div>
                </td>
                <td class="py-3.5 px-4 text-muted-foreground">
                  <div>{{ item.dosageForm }}</div>
                  <span class="px-2 py-0.5 rounded text-[10px] font-semibold bg-secondary text-secondary-foreground">
                    {{ item.category }}
                  </span>
                </td>
                <td class="py-3.5 px-4">
                  <span
                    [ngClass]="item.totalQuantityOnHand <= item.reorderLevel ? 'text-destructive font-bold' : 'text-foreground font-semibold'"
                ...
                <td class="py-3.5 px-4">
                  <div class="space-y-1">
                    <div *ngFor="let b of item.batches" class="text-[11px] font-mono flex items-center gap-2">
                      <span class="px-1.5 py-0.5 rounded bg-muted font-bold text-foreground">#{{ b.batchNumber }}</span>
                      <span class="text-muted-foreground">Exp: {{ b.expiryDate }}</span>
                      <span class="text-emerald-600 font-semibold">({{ b.quantityOnHand }} units)</span>
                      <span class="text-[10px] text-muted-foreground">[{{ b.location }}]</span>
                    </div>
                    <div *ngIf="!item.batches || item.batches.length === 0" class="text-muted-foreground text-[11px] italic">
                      No active batches
                    </div>
                  </div>
                </td>
      ...
      <div *ngIf="showReceiveModal()" class="fixed inset-0 z-50 bg-background/80 backdrop-blur-xs flex items-center justify-center p-4">
        <div class="w-full max-w-lg rounded-2xl border border-border bg-card p-6 shadow-2xl space-y-4">
          <div class="flex justify-between items-center border-b border-border pb-3">
            ...
            <button (click)="showReceiveModal.set(false)" class="p-1 rounded-lg text-muted-foreground hover:text-foreground">
          ...
          <div class="space-y-3 text-xs">
            <div>
              ...
              <input type="text" [(ngModel)]="receiveForm.medicationName" placeholder="e.g. Amoxicillin Trihydrate 500mg" class="w-full px-3 py-2 rounded-lg border border-input bg-background" />
            </div>

            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="block font-semibold text-foreground mb-1">Batch Number *</label>
                <input type="text" [(ngModel)]="receiveForm.batchNumber" placeholder="e.g. BAT-2026-X9" class="w-full px-3 py-2 rounded-lg border border-input bg-background font-mono uppercase" />
              </div>
              <div>
                <label class="block font-semibold text-foreground mb-1">Expiration Date *</label>
                <input type="date" [(ngModel)]="receiveForm.expiryDate" class="w-full px-3 py-2 rounded-lg border border-input bg-background font-mono" />
              </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="number" [(ngModel)]="receiveForm.quantity" class="w-full px-3 py-2 rounded-lg border border-input bg-background font-mono" />
              </div>
              <div>
                <label class="block font-semibold text-foreground mb-1">Unit Cost Price ($)</label>
                <input type="number" [(ngModel)]="receiveForm.unitPrice" class="w-full px-3 py-2 rounded-lg border border-input bg-background font-mono" />
              </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="text" [(ngModel)]="receiveForm.manufacturer" placeholder="e.g. Sun Pharma" class="w-full px-3 py-2 rounded-lg border border-input bg-background" />
              </div>
              <div>
                <label class="block font-semibold text-foreground mb-1">Storage Shelf Location</label>
                <input type="text" [(ngModel)]="receiveForm.location" placeholder="e.g. Shelf A-2" class="w-full px-3 py-2 rounded-lg border border-input bg-background font-mono" />
              </div>
            </div>
          </div>

          <div class="flex justify-end gap-2 pt-3 border-t border-border">
            <button (click)="showReceiveModal.set(false)" class="px-4 py-2 rounded-lg border border-border text-xs font-semibold">Cancel</button>
            <button (click)="submitReceiveStock()" [disabled]="!receiveForm.medicationName || !receiveForm.batchNumber" class="px-4 py-2 rounded-lg bg-purple-600 text-white text-xs font-semibold hover:bg-purple-700 transition-colors disabled:opacity-50">
...
export class PharmacistInventoryComponent implements OnInit {
  ...
  submitReceiveStock(): void {
    this.apiService.receiveStockBatch(this.receiveForm).subscribe({
      next: () => {
        toast.success(`Received ${this.receiveForm.quantity} units of ${this.receiveForm.medicationName}.`);
        this.showReceiveModal.set(false);
        this.loadInventory();
      },
      error: () => {
        let item = this.inventory().find((i) => i.medicationName.toLowerCase().includes(this.receiveForm.medicationName.toLowerCase()));
        if (item) {
