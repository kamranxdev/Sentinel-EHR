    ...
    <div class="space-y-6 font-sans">
      ...
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pb-4 border-b border-border">
        <div class="flex items-center gap-4">
          <div class="size-12 rounded-xl bg-gradient-to-br from-indigo-500/20 via-indigo-500/10 to-indigo-500/5 text-indigo-600 flex items-center justify-center shrink-0 border border-indigo-500/20 shadow-xs">
          ...
          <div>
            <div class="flex items-center gap-2 flex-wrap">
              ...
              <span hlmBadge variant="secondary" class="text-[10px] uppercase font-mono tracking-wider bg-indigo-500/10 text-indigo-600 border-indigo-500/30">
                CLINICAL PHARMACIST
              </span>
            </div>
            <p class="text-xs text-muted-foreground mt-0.5 flex items-center gap-2 flex-wrap">
              ...
              <span>•</span>
              <span>Prescription safety verification, drug-drug interaction screening, batch dispensing, & inventory control</span>
            </p>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <a routerLink="/pharmacist/erx" class="p-4 rounded-xl border border-border bg-card shadow-xs hover:border-amber-500/40 transition-all group">
          <div class="flex items-center justify-between">
            <span class="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Pending Review</span>
          ...
          <div class="mt-2 flex items-baseline justify-between">
            <div class="text-2xl font-bold text-amber-600 font-mono">{{ pendingVerificationCount() }}</div>
        ...
        <!-- Ready to Dispense -->
        <a routerLink="/pharmacist/dispense" class="p-4 rounded-xl border border-border bg-card shadow-xs hover:border-indigo-500/40 transition-all group">
          <div class="flex items-center justify-between">
            <span class="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Ready to Dispense</span>
          ...
          <div class="mt-2 flex items-baseline justify-between">
            <div class="text-2xl font-bold text-indigo-600 font-mono">{{ verifiedOrdersCount() }}</div>
        ...
        <!-- Low Stock Items -->
        <a routerLink="/pharmacist/inventory" class="p-4 rounded-xl border border-border bg-card shadow-xs hover:border-destructive/40 transition-all group">
          <div class="flex items-center justify-between">
            <span class="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Low Stock Alert</span>
            <div class="size-9 rounded-lg bg-destructive/10 text-destructive flex items-center justify-center group-hover:scale-105 transition-transform">
        ...
        <div class="p-4 rounded-xl border border-border bg-card shadow-xs">
          <div class="flex items-center justify-between">
            <span class="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Dispensed Today</span>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <a
          ...
          <div class="space-y-3">
            <div class="flex items-center justify-between">
              ...
              <span class="text-xs font-semibold text-indigo-600 group-hover:translate-x-0.5 transition-transform flex items-center gap-1">
                Open Queue <ng-icon name="lucideArrowRight" size="14" />
              </span>
            </div>
            <div>
              <h3 class="text-base font-semibold text-foreground group-hover:text-indigo-600 transition-colors">Prescription Verification & Safety</h3>
              <p class="text-xs text-muted-foreground mt-1 leading-relaxed">
                Review inpatient and outpatient medication orders with automated allergy checks, drug-drug interaction warnings, and 3-way decisioning (Approve, Clarify, Reject).
              </p>
            </div>
          </div>
          <div class="pt-3 border-t border-border flex items-center justify-between text-xs">
            <span class="text-muted-foreground font-mono">Pending: {{ pendingVerificationCount() }}</span>
        ...
        <a
          ...
          <div class="space-y-3">
            ...
            <div>
              <h3 class="text-base font-semibold text-foreground group-hover:text-emerald-600 transition-colors">Medication Dispensing & MAR</h3>
              <p class="text-xs text-muted-foreground mt-1 leading-relaxed">
                Dispense verified prescriptions by selecting active medication batch (FEFO/FIFO), auto-deducting physical stock, and dispatching billable charges to Revenue Cycle.
              </p>
            </div>
          </div>
          <div class="pt-3 border-t border-border flex items-center justify-between text-xs">
            <span class="text-muted-foreground font-mono">Verified: {{ verifiedOrdersCount() }}</span>
        ...
        <a
          ...
          <div class="space-y-3">
            <div class="flex items-center justify-between">
              ...
              <span class="text-xs font-semibold text-purple-600 group-hover:translate-x-0.5 transition-transform flex items-center gap-1">
                View Stock <ng-icon name="lucideArrowRight" size="14" />
              </span>
            </div>
            <div>
              <h3 class="text-base font-semibold text-foreground group-hover:text-purple-600 transition-colors">Inventory, Batches & Expiry</h3>
              <p class="text-xs text-muted-foreground mt-1 leading-relaxed">
                Track pharmaceutical stock levels, batch numbers, manufacturer details, shelf storage locations, expiry dates, and receive/adjust stock shipments.
              </p>
            </div>
          </div>
          <div class="pt-3 border-t border-border flex items-center justify-between text-xs">
            <span class="text-muted-foreground font-mono">Catalog: {{ inventory().length }} items</span>
      ...
      <div class="bg-card rounded-2xl border border-border shadow-xs overflow-hidden">
        <div class="p-4 border-b border-border bg-muted/20 flex items-center justify-between">
          <div>
            ...
            <p class="text-xs text-muted-foreground">Physician medication orders awaiting clinical pharmacist sign-off.</p>
          </div>
          <a routerLink="/pharmacist/erx" class="text-xs font-semibold text-indigo-600 hover:underline">
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-xs text-left">
            ...
            <tbody class="divide-y divide-border">
              <tr *ngFor="let order of pendingOrders().slice(0, 5)" class="hover:bg-muted/30 transition-colors">
                <td class="py-3.5 px-4 font-semibold text-foreground">
                  <div class="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div class="text-[11px] font-normal text-muted-foreground">{{ order.dosage }} &bull; Qty: {{ order.quantity }}</div>
                    </div>
                  </div>
                </td>
                <td class="py-3.5 px-4">
                  <div class="font-medium text-foreground">{{ order.patient?.fullName || 'Patient' }}</div>
                  <div class="text-[10px] font-mono text-muted-foreground">{{ order.patient?.patientCode || order.patientId }}</div>
                </td>
                <td class="py-3.5 px-4 text-muted-foreground">
                  <div>{{ order.route }} &bull; {{ order.frequency }}</div>
                  <div class="text-[10px]">{{ order.instructions || 'Standard administration' }}</div>
                </td>
                <td class="py-3.5 px-4 font-mono text-muted-foreground text-[11px]">
                  {{ order.orderedAt | date:'short' }}
                </td>
                <td class="py-3.5 px-4">
                  <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-amber-500/15 text-amber-600 border border-amber-500/30">
                    PENDING VERIFICATION
                  </span>
                </td>
...
export class PharmacistDashboardComponent implements OnInit {
  ...
  pendingOrders = computed(() =>
    this.orders().filter((o) => o.status === 'PENDING_VERIFICATION' || o.status === 'PRESCRIBED' || o.status === 'ORDERED')
  );
