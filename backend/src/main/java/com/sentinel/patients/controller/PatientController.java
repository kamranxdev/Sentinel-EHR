...
import com.sentinel.patients.mapper.PatientMapper;
...
public class PatientController {
    ...
    public ResponseEntity<PatientResponseDTO> getPatientById(@PathVariable Long id, Authentication auth) {
    ...
    public ResponseEntity<PatientResponseDTO> registerIntakePatient(@Valid @RequestBody PatientRequestDTO payload, Authentication auth) {
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(patientMapper.toResponseDTO(saved));
    }

    @GetMapping("/me")
    public ResponseEntity<PatientResponseDTO> getMyPatientProfile(Authentication auth) {
    ...
    public ResponseEntity<PatientResponseDTO> getPatientByUserId(@PathVariable Long userId, Authentication auth) {
    ...
    public ResponseEntity<PatientResponseDTO> createPatient(@Valid @RequestBody PatientRequestDTO payload, Authentication auth) {
        Patient patientEntity = patientMapper.toEntity(payload);
        Patient saved = patientService.createPatient(patientEntity);
    ...
    @PutMapping("/{id}")
    @PreAuthorize("@patientSecurityService.canAccessPatient(authentication, #id)")
    public ResponseEntity<PatientResponseDTO> updatePatient(@PathVariable Long id, @Valid @RequestBody PatientRequestDTO payload, Authentication auth) {
        Patient updatedEntity = patientMapper.toEntity(payload);
